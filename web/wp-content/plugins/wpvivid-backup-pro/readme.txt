...
== Changelog ==
= 2.2.49 =
- Improved the handling of sensitive data during backup and restore processes.
- Optimized plugin code for improved performance and stability.
- Successfully tested with WordPress 7.0.4.
= 2.2.48 =
- Added multilingual support and provided translation templates for translators.
- Added an option to reset the default backup exclusion directories.
- Added support for excluding the backup directory when exporting and importing settings.
- Fixed an issue where filtering database tables could reset table selections.
- Fixed some vulnerabilities in the plugin code.
= 2.2.47 =
- Fixed: Database restore could fail due to duplicate foreign key constraint names.
- Fixed: Backups in Cloudflare R2 could not be detected during backup scans in some cases.
- Fixed: Uploading backups to multiple remote storage destinations could fail if the Nextcloud upload encountered an error.
- Improved plugin performance by optimizing frontend asset loading.
= 2.2.46 =
- Added an option to encrypt and securely store FTP storage passwords.
- Fixed: Google Drive refresh token generation could fail in some environments.
- Fixed: Adding Nextcloud storage could fail in some environments.
- Fixed some PHP deprecation warnings in PHP 8.5 environments.
- Fixed some UI bugs.
- Successfully tested with WordPress 7.0.
= 2.2.45 =
- Added support for configuring Google Drive upload chunk size.
- Added options to show/hide addons on the plugin dashboard.
- Added wp-content/duplicator-backups to the default excluded directories.
- Fixed an issue where some plugin data was not removed after uninstalling.
- Fixed an issue where pCloud rollback database backups did not follow retention settings.
- Updated several UI descriptions and interface texts.
= 2.2.44 =
- Added plugin dependencies between WPvivid pro and free versions.
- Added support for resumable uploads for Google Drive remote storage.
- Fixed: Migrating a MainWP child site to another would cause the destination site to lose its connection with the MainWP Dashboard.
- Fixed an issue where some backup times displayed incorrectly due to timezone differences.
- Fixed: Restoration could fail under specific conditions when using PDO mode.
= 2.2.43 =
