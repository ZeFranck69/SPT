...
class WPvivid_Storage_List extends WP_List_Table
{
    ...
    {
        $columns = array();
        $columns['cb'] = __( 'cb', 'wpvivid-backup-pro' );
        $columns['wpvivid_storage_type'] = __( 'Provider', 'wpvivid-backup-pro' );
        $columns['wpvivid_storage_actions'] =__( 'Actions', 'wpvivid-backup-pro'  );
        $columns['wpvivid_storage_comment'] = __( 'Comment', 'wpvivid-backup-pro' );
        return $columns;
    }

    public function  column_cb( $storage )
    {
        $html='<input type="checkbox" name="remote_storage" value="'.esc_attr($storage['key']).'" '.esc_attr($storage['check_status']).' />';
        echo $html;
    }
    ...
    public function _column_wpvivid_storage_type( $storage )
    {
        ...
        echo '<span>'.__($storage_type, 'wpvivid-backup-pro').'</span>';
    ...
    public function _column_wpvivid_storage_actions( $storage )
    {
        $html='<td class="tablelistcolumn">
                    <span class="dashicons dashicons-admin-generic wpvivid-dashicons-grey wpvivid-schedule-edit" onclick="click_retrieve_remote_storage(\''.__($storage['key'], 'wpvivid-backup-pro').'\',\''.__($storage['type'], 'wpvivid-backup-pro').'\',\''.__($storage['name'], 'wpvivid-backup-pro').'\');" style="cursor: pointer;"></span>
                    <span class="dashicons dashicons-trash wpvivid-dashicons-grey wpvivid-schedule-delete" onclick="wpvivid_delete_remote_storage(\''.__($storage['key'], 'wpvivid-backup-pro').'\');" style="cursor: pointer;"></span>
                </td>';
        echo $html;
    }
    ...
    {
        $html='<td class="plugin-title column-primary"><label for="tablecell">'.__($storage['name'], 'wpvivid-backup-pro').'</label></td>';
        echo $html;
    }
        ...
        <table class="wp-list-table <?php echo implode( ' ', $this->get_table_classes() ); ?>">
            ...
            <tr>
                <th colspan="5" class="row-title">
                    <input class="button-primary" id="wpvivid_set_default_remote_storage" type="submit" name="choose-remote-storage" value="<?php esc_attr_e('Save Changes', 'wpvivid-backup-pro'); ?>"/>
                </th>
            </tr>
            </tfoot>
...
class WPvivid_Multi_Remote_addon
{
    ...
    public function __construct()
    {
        ...
        add_action('wp_ajax_wpvivid_generate_remote_crypt_key', array($this, 'generate_remote_crypt_key'));
        add_action('wp_ajax_wpvivid_test_remote_crypt_key', array($this, 'test_remote_crypt_key'));
        //
        //filters
    ...
    public function get_dashboard_menu($submenus,$parent_slug)
    {
        ...
        if($display)
        {
            ...
            $submenu['page_title'] = apply_filters('wpvivid_white_label_display', __('Cloud Storage', 'wpvivid-backup-pro'));
            $submenu['menu_title'] = __('Cloud Storage', 'wpvivid-backup-pro');
    ...
    public function get_toolbar_menus($toolbar_menus)
    {
        ...
        if($display) {
            ...
            $menu['title'] = __('Cloud Storage', 'wpvivid-backup-pro');
    ...
    public function edit_remote(){
        ...
        try
        {
            ...
            $old_remote = apply_filters('wpvivid_edit_remote_options_ex', $old_remote, $remote_options, $_POST['id']);
    ...
    public function delete_remote(){
        ...
        try
        {
            ...
            if (WPvivid_Setting::delete_remote_option($id))
            {
                ...
                $ret['result'] = 'failed';
                $ret['error'] = __('Fail to delete the remote storage, can not retrieve the storage infomation. Please try again.', 'wpvivid-backup-pro');
            }
            echo json_encode($ret);
        }
        catch (Exception $error)
    ...
    public function set_default_remote_storage_ex(){
        ...
        try {
            if (!isset($_POST['remote_storage']) || empty($_POST['remote_storage']) || !is_array($_POST['remote_storage'])) {
                ...
                $ret['error'] = __('Choose one storage from the list to be the default storage.', 'wpvivid-backup-pro');
    ...
    public function encrypt_remote_password($data){
        ...
        else if (isset($data['is_encrypt']) && intval($data['is_encrypt']) === 2) {
        ...
        if (isset($data['type']) && $data['type'] === 'ftp') {
            $data['use_wp_config_crypt'] = (isset($data['is_encrypt']) && intval($data['is_encrypt']) === 2) ? '1' : '0';
        }
        return $data;
    }
    ...
    {
        global $wpvivid_backup_pro;
        $has_remote_password_crypt = false;
        $imported_key = '';
        ...
        if(isset($json['data']['wpvivid_upload_setting']) && !empty($json['data']['wpvivid_upload_setting']))
        {
            foreach ($json['data']['wpvivid_upload_setting'] as $key=>$value)
            {
                if($key !== 'remote_selected')
                {
                    if(isset($value['is_encrypt']) && intval($value['is_encrypt']) === 2)
                    {
                        $has_remote_password_crypt = true;
                    }

                    if(isset($value['encrypt_method']) && $value['encrypt_method'] === 'remote_password_key')
                    {
                        $has_remote_password_crypt = true;
                    }

                    if(isset($value['password']) && is_string($value['password']) && strpos($value['password'], 'wpvivid_remote_v1:') === 0)
                    {
                        $has_remote_password_crypt = true;
                    }

                    if(isset($value['secret']) && is_string($value['secret']) && strpos($value['secret'], 'wpvivid_remote_v1:') === 0)
                    {
                        $has_remote_password_crypt = true;
                    }
        ...
        if(isset($json['data']['wpvivid_remote_password_crypt']['key']))
        {
            $imported_key = $json['data']['wpvivid_remote_password_crypt']['key'];
        }

        if($has_remote_password_crypt)
        {
            update_option('wpvivid_remote_password_crypt_import_notice', array(
                'show'         => 1,
                'key_included' => empty($imported_key) ? 0 : 1,
    ...
    public function remote_storage_backup_retention($type, $action)
    {
        ...
        <tr>
            <td colspan=2>
                <label><input class="<?php echo esc_attr($checkbox_classname); ?>" type="checkbox" option="<?php echo esc_attr($option); ?>" name="use_remote_retention" onclick="wpvivid_check_special_retention(this);"><?php esc_html_e('Enable a special rule of backup retention for the storage', 'wpvivid-backup-pro'); ?>
            </td>
        </tr>
        ...
        <tr class="<?php echo esc_attr($tr_classname); ?>" style="display: none;">
            ...
            <td class="column-description desc">
                <div class="wpvivid-storage-form-desc">
                    <i><?php esc_html_e('(Manual Backup + General Schedule) File backups retained.', 'wpvivid-backup-pro'); ?></i>
                </div>
            </td>
        </tr>
        ...
        <tr class="<?php echo esc_attr($tr_classname); ?>" style="display: none;">
            ...
            <td class="column-description desc">
                <div class="wpvivid-storage-form-desc">
                    <i><?php esc_html_e('(Manual Backup + General Schedule) Database backups retained.', 'wpvivid-backup-pro'); ?></i>
                </div>
            </td>
        </tr>
        ...
        <tr class="<?php echo esc_attr($tr_classname); ?>" style="display: none;">
            ...
            <td class="column-description desc">
                <div class="wpvivid-storage-form-desc">
                    <i><?php esc_html_e('(Incremental Backups) Cycles of incremental backups retained.', 'wpvivid-backup-pro'); ?></i>
                </div>
            </td>
        </tr>
        ...
        <tr class="<?php echo esc_attr($tr_classname); ?>" style="display: none;">
            ...
            <td class="column-description desc">
                <div class="wpvivid-storage-form-desc">
                    <i><?php esc_html_e('(Rollback) Rollback Backups retained.', 'wpvivid-backup-pro'); ?></i>
                </div>
            </td>
        </tr>
    ...
    public function init_page_ex()
    {
                            ...
                            <div class="wpvivid-backup">
                                <div class="wpvivid-welcome-bar wpvivid-clear-float">
                                    <div class="wpvivid-welcome-bar-left">
                                        <p><span class="dashicons dashicons-admin-site-alt3 wpvivid-dashicons-large wpvivid-dashicons-green"></span><span class="wpvivid-page-title"><?php esc_html_e('Cloud Storage', 'wpvivid-backup-pro'); ?></span></p>
                                        <p><span class="about-description"><?php esc_html_e('Connect to your cloud storage accounts and set a custom backup folder in each remote storage.', 'wpvivid-backup-pro'); ?></span></p>
                                    </div>
                                ...
                                {
                                    $success_msg = __('You have successfully updated the account information of your remote storage.','wpvivid-backup-pro');
                                    $notice_style='';
                                }
                                if(isset($_REQUEST['delete_remote']))
                                {
                                    $success_msg = __('You have successfully remove your remote storage.','wpvivid-backup-pro');
                                    $notice_style='';
                                }
                                if(isset($_REQUEST['add_remote']))
                                {
                                    $success_msg = __('You have successfully added a remote storage.','wpvivid-backup-pro');
                                    $notice_style='';
                                }
                                if(isset($_REQUEST['change_default']))
                                {
                                    $success_msg = __('You have successfully changed your default remote storage.','wpvivid-backup-pro');
                                    $notice_style='';
                                }
                                ?>
                                <div class="wpvivid-v2-padding" id="wpvivid_remote_storage_notice" style="padding-bottom: 0; <?php echo esc_attr($notice_style); ?>">
                                ...
                                <div class="wpvivid-canvas wpvivid-clear-float wpvivid-remote-storage-tab">
                                    ...
                                    $tabs['cloud_storage']['title']=__('Cloud Storage', 'wpvivid-backup-pro');
                                    ...
                                    $args['can_delete']=1;
                                    $args['hide']=1;
                                    $tabs['add_storage']['title']=__('Add Cloud Storage', 'wpvivid-backup-pro');
                                    ...
                                    $args['can_delete']=1;
                                    $args['hide']=1;
                                    $tabs['edit_storage']['title']=__('Edit Cloud Storage', 'wpvivid-backup-pro');
        ...
        <script>
            function wpvivid_check_remote_crypt_before_save(container)
            {
                if (container.length === 0) {
                    return true;
                }

                var use_crypt = container.find('input[name="use_wp_config_crypt"]').prop('checked');

                if (!use_crypt) {
                    return true;
                }

                if (container.find('input[name="remote_crypt_test_passed"]').val() !== '1') {
                    alert(wpvivid_ajax_object_addon.i18n.test_encryption_before_adding_remote);
                    return false;
                }
            ...
            {
                if (storage_type === 'ftp') {
                    if (!wpvivid_check_remote_crypt_before_save(jQuery('#storage_account_ftp'))) {
                        return;
                    }
                }
    ...
    public function output_cloud_storage()
    {
        ...
        <script>
            ...
            function wpvivid_edit_remote_storage() {
                if (wpvivid_editing_storage_type === 'ftp') {
                    if (!wpvivid_check_remote_crypt_before_save(jQuery('#remote_storage_edit_ftp'))) {
                        return;
                    }
                }
                    ...
                    try
                    {
                        ...
                        if (jsonarray.result === 'success')
                        {
                            ...
                            jQuery('input:text[option=edit-'+jsonarray.type+']').each(function(){
                                ...
                                if (jsonarray.type === 'googledrive') {
                                    ...
                                    if(key === 'download_chunk_size'){
                                        if (typeof jsonarray[key] === 'undefined' || jsonarray[key] === null || jsonarray[key] === '' || isNaN(jsonarray[key])) {
                                            // Optional compatibility if an old edit page still has name="chunk_size".
                                            if (typeof jsonarray['chunk_size'] !== 'undefined' && jsonarray['chunk_size'] !== null && jsonarray['chunk_size'] !== '' && !isNaN(jsonarray['chunk_size'])) {
                                                jsonarray[key] = jsonarray['chunk_size'] / 1024 / 1024;
                                            }
                                            else {
                                                jsonarray[key] = 2;
                                            }
                            ...
                            jQuery('input:checkbox[option=edit-'+jsonarray.type+']').each(function() {
                                ...
                                if(jsonarray.type === 'ftp' && typeof jsonarray[key] === 'undefined')
                                {
                                    value=false;
                                }
                                else if(jsonarray[key] == '0'){
                                    value = false;
                                }
                                else{
                                    value = true;
    ...
    public function retrieve_remote_ex()
    {
        ...
        $ret['error'] = __('Failed to get the remote storage information. Please try again later.', 'wpvivid-backup-pro');
        foreach ($remoteslist as $key => $value)
        {
            if ($key == $id)
            {
    ...
    public function generate_remote_crypt_key()
    {
        global $wpvivid_backup_pro;
        $wpvivid_backup_pro->ajax_check_security('wpvivid-can-mange-remote');
        ...
        if (!class_exists('WPvivid_Remote_Password_Crypt_Ex')) {
            echo json_encode(array(
                ...
                'error'  => __('Failed to load the encryption class. Please ensure WPvivid Backup Pro is up to date.','wpvivid-backup-pro')
            ));
            die();
        }

        $crypt = new WPvivid_Remote_Password_Crypt_Ex();
        $has_current_define = $crypt->has_config_key();
        $key = $crypt->generate_key();

        if ($key === false || $key === '') {
            echo json_encode(array(
                'result' => WPVIVID_PRO_FAILED,
                'error'  => __('Failed to generate the encryption key.','wpvivid-backup-pro')
            ));
            die();
        }

        echo json_encode(array(
            'result'             => WPVIVID_PRO_SUCCESS,
            'key'                => $key,
            'key_hash'           => $crypt->get_key_hash_from_base64($key),
            'define'             => $crypt->get_define_line($key),
            'has_current_define' => $has_current_define ? 1 : 0
        ));

        die();
    }

    public function test_remote_crypt_key()
    {
        global $wpvivid_backup_pro;
        ...
        if (!isset($_POST['password'])) {
            echo json_encode(array(
                'result' => WPVIVID_PRO_FAILED,
                'error'  => __('The remote storage password is required.','wpvivid-backup-pro')
            ));
            die();
        }
        ...
        $generated_key_hash = '';
        if (isset($_POST['generated_key_hash'])) {
            $generated_key_hash = sanitize_text_field(wp_unslash($_POST['generated_key_hash']));
        }
        ...
        if (!$crypt->load_key_from_config()) {
            echo json_encode(array(
                'result' => WPVIVID_PRO_FAILED,
                'error'  => __('The encryption key is missing or invalid in wp-config.php. Please add a valid key.','wpvivid-backup-pro')
            ));
            die();
        }

        if ($generated_key_hash !== '') {
            $loaded_key_hash = $crypt->get_loaded_key_hash();

            if ($loaded_key_hash === '' || $loaded_key_hash !== $generated_key_hash) {
                echo json_encode(array(
                    'result' => WPVIVID_PRO_FAILED,
                    'error'  => __('The key in wp-config.php does not match the generated key. Please update wp-config.php and try again.','wpvivid-backup-pro')
                ));
                die();
            }
        }
        ...
        if ($encrypted === false || $encrypted === '') {
            echo json_encode(array(
                'result' => WPVIVID_PRO_FAILED,
                'error'  => __('Failed to encrypt the remote storage password. Please test the encryption first.','wpvivid-backup-pro')
            ));
            die();
        }
        ...
        if ($decrypted !== $password) {
            echo json_encode(array(
                'result' => WPVIVID_PRO_FAILED,
                'error'  => __('Failed to verify the encrypted remote storage password. Please test the encryption first.','wpvivid-backup-pro')
            ));
            die();
        }
        ...
        echo json_encode(array(
            'result'        => WPVIVID_PRO_SUCCESS,
            'notice'        => __('Encryption test passed. You can now add the remote storage.','wpvivid-backup-pro'),
            'key_hash'      => $crypt->get_loaded_key_hash(),
    ...
    public function output_add_storage()
    {
        ...
        <div id="wpvivid_archieve_remote_info" style="margin-top: 10px;">
            <div style="float: left; height: 20px; line-height: 20px; margin-top: 4px;"><?php esc_html_e('Retrieving the information of remote storage', 'wpvivid-backup-pro'); ?></div>
    ...
    public function init_page()
    {
        ...
        <div class="wrap wpvivid-canvas">
            <div id="icon-options-general" class="icon32"></div>
            <h1><?php printf( esc_html__('%s Plugins - Cloud Storage', 'wpvivid-backup-pro'), esc_html(apply_filters('wpvivid_white_label_display', 'WPvivid')) ); ?></h1>
                        ...
                        <div class="meta-box-sortables ui-sortable">
                            <div class="wpvivid-backup">
                                ...
                                <div class="wpvivid-canvas wpvivid-clear-float wpvivid-remote-storage-tab">
                                    ...
                                    $args['div_style']='display:block;';
                                    $args['is_parent_tab']=0;
                                    $tabs['cloud_storage_google_drive']['title']=__('Google Drive', 'wpvivid-backup-pro');
                                    ...
                                    $args['div_style']='';
                                    $tabs['cloud_storage_dropbox']['title']=__('Dropbox', 'wpvivid-backup-pro');
                                    ...
                                    $tabs['cloud_storage_pcloud']['title']=__('pCloud', 'wpvivid-backup-pro');
                                    ...
                                    $tabs['cloud_storage_onedrive']['title']=__('Microsoft OneDrive', 'wpvivid-backup-pro');
                                    ...
                                    $tabs['cloud_storage_amazons3']['title']=__('Amazon S3', 'wpvivid-backup-pro');
                                    ...
                                    $tabs['cloud_storage_digitalocean']['title']=__('S3 Compatible Storage', 'wpvivid-backup-pro');
                                    ...
                                    $args['span_class']='ftp';
                                    $tabs['cloud_storage_ftp']['title']=__('FTP', 'wpvivid-backup-pro');
                                    ...
                                    $tabs['cloud_storage_ftp']['title']=__('FTP2', 'wpvivid-backup-pro');
                                    ...
                                    $tabs['cloud_storage_sftp']['title']=__('SFTP', 'wpvivid-backup-pro');
                                    ...
                                    $tabs['cloud_storage_wasabi']['title']=__('Wasabi', 'wpvivid-backup-pro');
                                    ...
                                    $args['span_class']='b2';
                                    $tabs['cloud_storage_b2']['title']=__('Backblaze Storage', 'wpvivid-backup-pro');
                                    ...
                                    $tabs['cloud_storage_webdav']['title']=__('WebDav Storage', 'wpvivid-backup-pro');
                                    ...
                                    $tabs['cloud_storage_nextcloud']['title']=__('Nextcloud Storage', 'wpvivid-backup-pro');
    ...
    public function output_storages_list()
    {
        ...
        <div style="margin-top:10px;">
            <p>
                <strong>
                    <?php esc_html_e('Please choose one storage to save your backups (remote storage)', 'wpvivid-backup-pro'); ?>
                </strong>
            </p>
        </div>
