...
class WPvivid_Backup_Restore_Page_addon
{
    ...
    public function wpvivid_handle_backup_succeed($task)
    {
        ...
        else if($task['type']=='Cron')
        {
            ...
            WPvivid_Pro_Option::update_general_schedule_data($task_msg);
        }
        //mu
        $task_msg = WPvivid_taskmanager::get_task($task['id']);
    ...
    public function wpvivid_handle_backup_failed($task, $need_set_low_resource_mode)
    {
        ...
        {
            $notice_msg1 = __('Backup failed, it seems due to insufficient server resource or hitting server limit. Please navigate to Settings > Advanced > ', 'wpvivid-backup-pro');
            $notice_msg2 = __('optimization mode for web hosting/shared hosting', 'wpvivid-backup-pro');
            $notice_msg3 = __(' to enable it and try again', 'wpvivid-backup-pro');
            $tasks[$task['id']]['error_msg'] = '<div class="notice notice-error inline"><p>' . esc_html($notice_msg1) . '<strong>' . esc_html($notice_msg2) . '</strong>' . esc_html($notice_msg3) . '</p></div>';
            $tasks[$task['id']]['tmp_msg']=$notice_msg1.$notice_msg2.$notice_msg3;
        }
        else if($need_set_low_resource_mode)
        {
            $notice_msg = __('Backup failed, it seems due to insufficient server resource or hitting server limit.', 'wpvivid-backup-pro');
            $debug_url = $admin_url . 'admin.php?page=' . apply_filters('wpvivid_white_label_plugin_name', 'wpvivid-debug');
            $tasks[$task['id']]['error_msg'] = '<div class="notice notice-error inline"><p>' . sprintf(
                wp_kses_post(__('%1$s, Please switch to <a href="%2$s">Website Info</a> page to send us the debug information.', 'wpvivid-backup-pro')),
                esc_html($notice_msg),
                esc_url($debug_url)
            ) . '</p></div>';
            ...
            $notice_msg = sprintf(__('Backup error: %1$s, task id: %2$s', 'wpvivid-backup-pro'), $task['status']['error'], $task['id']);
            $debug_url = $admin_url . 'admin.php?page=' . apply_filters('wpvivid_white_label_plugin_name', 'wpvivid-debug');
            $tasks[$task['id']]['error_msg'] = '<div class="notice notice-error inline"><p>' . sprintf(
                wp_kses_post(__('%1$s, Please switch to <a href="%2$s">Website Info</a> page to send us the debug information.', 'wpvivid-backup-pro')),
                esc_html($notice_msg),
    ...
    public function wpvivid_backuppage_add_backup_type($html, $type_name)
    {
        ...
        <label style="padding-right:2em;">
            <input type="radio" option="backup" name="<?php echo esc_attr($type_name); ?>" value="files+db" checked="checked">
        ...
        <label>
            <input type="radio" option="backup" name="<?php echo esc_attr($type_name); ?>" value="custom">
    ...
    public function check_backup_options_valid($ret,$data,$backup_method)
    {
        ...
        {
            $ret['error']=__('A backup type is required.', 'wpvivid-backup-pro');
            return $ret;
        }

        if(!isset($data['local']) && !isset($data['remote']))
        {
            $ret['error']=__('Choose at least one storage location for backups.', 'wpvivid-backup-pro');
            return $ret;
        }
            ...
            $other_table_html .= '<div class="wpvivid-custom-database-other-table-header" style="border:1px solid #e5e5e5;">
                                        <div>
                                            <div style="float: left; margin-right: 10px;">
                                                ...
                                                <span class="wpvivid-checkbox-checkmark"></span>'.esc_html__('Tables created by plugins or themes', 'wpvivid-backup-pro').'
                                                </label>
                                            </div>
                                            <span class="dashicons dashicons-editor-help wpvivid-dashicons-editor-help wpvivid-tooltip">
                                                <div class="wpvivid-bottom">
        ...
        try {
            if (isset($_POST['database_info']) && !empty($_POST['database_info']) && is_string($_POST['database_info'])) {
                ...
                if($ret['result']===WPVIVID_PRO_SUCCESS)
                {
                    ...
                    if (!isset($history['additional_database_option']) || !is_array($history['additional_database_option']))
                    {
                        $history['additional_database_option'] = array();
                    }

                    $history['additional_database_option']['db_host'] = $db_host;
                    $history['additional_database_option']['db_user'] = $db_user;
                    unset($history['additional_database_option']['db_pass']);
                    unset($history['additional_database_option']['additional_database_list']);
        ...
        <div class="wpvivid-one-coloum" id="wpvivid_postbox_backup_percent" style="display: none;">
            <div class="wpvivid-one-coloum wpvivid-workflow wpvivid-clear-float">
                ...
                <p>
                    ...
                    <span class="dashicons dashicons-admin-page wpvivid-dashicons-green"></span><span><?php esc_html_e('Total Size:', 'wpvivid-backup-pro'); ?></span><span><?php esc_html_e('N/A', 'wpvivid-backup-pro'); ?></span>
                    <span class="dashicons dashicons-upload wpvivid-dashicons-blue"></span><span><?php esc_html_e('Uploaded:', 'wpvivid-backup-pro'); ?></span><span><?php esc_html_e('N/A', 'wpvivid-backup-pro'); ?></span>
                    <span class="dashicons dashicons-plugins-checked wpvivid-dashicons-green"></span><span><?php esc_html_e('Speed:', 'wpvivid-backup-pro'); ?></span><span><?php esc_html_e('N/A', 'wpvivid-backup-pro'); ?></span>
                    <span class="dashicons dashicons-networking wpvivid-dashicons-green"></span><span><?php esc_html_e('Network Connection:', 'wpvivid-backup-pro'); ?></span><span><?php esc_html_e('OK', 'wpvivid-backup-pro'); ?></span>
                </p>
                <p><span class="dashicons dashicons-welcome-write-blog wpvivid-dashicons-grey"></span><span><?php esc_html_e('Action:', 'wpvivid-backup-pro'); ?></span><span><?php esc_html_e('running', 'wpvivid-backup-pro'); ?></span></p>
        ...
        foreach ($tasks as $task)
        {
            ...
            if($info['status']['str']=='ready'||$info['status']['str']=='running'||$info['status']['str']=='wait_resume'||$info['status']['str']=='no_responds')
            {
                ...
                $ret['progress_html'] = '<div class="wpvivid-one-coloum wpvivid-workflow wpvivid-clear-float">
                                            <p><span class="wpvivid-span-progress"><span class="wpvivid-span-processed-progress wpvivid-span-processed-percent-progress" style="width:'.$info['task_info']['backup_percent'].'">'.$info['task_info']['backup_percent'].' '.esc_html__('completed', 'wpvivid-backup-pro').'</span></span></p>
                                            <p>
    ...
    public function init_page()
    {
        ...
        <div class="wrap wpvivid-canvas">
            ...
            <h1><?php printf( esc_html__('%s Plugins - Manual Backup', 'wpvivid-backup-pro'), esc_html(apply_filters('wpvivid_white_label_display', 'WPvivid')) ); ?></h1>
                                        ...
                                        <div class="wpvivid-one-coloum wpvivid-workflow wpvivid-clear-float">
                                            <div style="">
                                                ...
                                                <div style="padding-left:2em;">
                                                    <label class="">
                                                        <input type="radio" option="backup" name="backup_to" value="local" <?php echo esc_attr($default_backup_local); ?> />Backup to localhost
                                                    </label>
                                                    ...
                                                    if( apply_filters('wpvivid_current_user_can',true,'wpvivid-can-backup-remote'))
                                                    {
                                                        ?>
                                                        <label class="">
                                                            <input type="radio" option="backup" name="backup_to" value="remote" <?php echo esc_attr($default_backup_remote); ?> />Backup to remote storage
                                                        </label>
                                            ...
                                            <div style="margin-bottom:-1em;border-top:1px solid #f1f1f1;padding-top:1em;">
                                                <input class="button-primary" style="width: 200px; height: 50px; font-size: 20px; margin-bottom: 10px; pointer-events: auto; opacity: 1;" id="wpvivid_quickbackup_btn" type="submit" value="<?php esc_attr_e('Backup Now', 'wpvivid-backup-pro'); ?>">
                                            ...
                                            <!--<div class="wpvivid-two-col">
                                                ...
                                                <fieldset>
                                                    <label class="wpvivid-radio">
                                                        <input type="radio" option="backup" name="backup_to" value="local" <?php echo esc_attr($default_backup_local); ?> />Save it to localhost
                                                    ...
                                                    if( apply_filters('wpvivid_current_user_can',true,'wpvivid-can-backup-remote'))
                                                    {
                                                        ...
                                                        <label class="wpvivid-radio">Send it to remote storage
                                                            <input type="radio" option="backup" name="backup_to" value="remote" <?php echo esc_attr($default_backup_remote); ?> />
                                                            <span class="wpvivid-radio-checkmark"></span>
                                                        </label>
            ...
            jQuery('#wpvivid_set_manual_prefix').on("keyup", function(){
                ...
                if(manual_prefix === ''){
                    ...
                    if (manual_prefix.match(reg)) {
                        ...
                        alert(wpvivid_ajax_object_addon.i18n.you_can_not_use_word_wpvivid_to_comment_the_backup);
                    }
                    else{
                        jQuery('#wpvivid_manual_prefix').html(manual_prefix);
                    }
                ...
                if(use_new_custom_backup_ui == '1'){
                    ...
                    if(jQuery('input:radio[option=backup][name=backup_files][value=custom]').prop('checked')){
                        ...
                        if(check_status === false){
                            alert(wpvivid_ajax_object_addon.i18n.please_select_at_least_one_item_under_the_additional_files);
                        }
                    }
                    else{
                        check_status = true;
            ...
            function wpvivid_check_additional_db_valid(type){
                ...
                if(use_new_custom_backup_ui == '1'){
                    ...
                    if(jQuery('input:radio[option=backup][name=backup_files][value=custom]').prop('checked')){
                        ...
                        if(check_status === false){
                            alert(wpvivid_ajax_object_addon.i18n.please_select_at_least_one_item_under_the_additional_database);
                        }
                    }
                    else{
                        check_status = true;
            ...
            function wpvivid_check_backup_option_avail(type){
                ...
                if(jQuery('#'+parent_id).find('.wpvivid-custom-database-part').prop('checked')){
                    ...
                    if(jQuery('#'+parent_id).find('.wpvivid-custom-database-check').prop('checked'))
                    {
                        ...
                        {
                            check_status = false;
                            alert(wpvivid_ajax_object_addon.i18n.please_select_at_least_one_table_to_back_up_or);
                            return check_status;
                        }
                    }
                    if(jQuery('#'+parent_id).find('.wpvivid-custom-additional-database-check').prop('checked'))
                    ...
                    if(!has_db_item){
                        check_status = false;
                        alert(wpvivid_ajax_object_addon.i18n.please_select_at_least_one_option_from_tables_in_the);
                        return check_status;
                    }
                }
                if(jQuery('#'+parent_id).find('.wpvivid-custom-file-part').prop('checked')){
                    ...
                    if(jQuery('#'+parent_id).find('.wpvivid-custom-additional-folder-check').prop('checked')){
                        ...
                        if(!has_additional_folder){
                            check_status = false;
                            alert(wpvivid_ajax_object_addon.i18n.please_select_at_least_one_additional_file_or_folder_under);
                            return check_status;
                        }
                    }
                    if(!has_file_item){
                        check_status = false;
                        alert(wpvivid_ajax_object_addon.i18n.please_select_at_least_one_option_under_the_option_files);
                        return check_status;
                    }
                }
        ...
        <div class="wpvivid-welcome-bar wpvivid-clear-float">
            ...
            <div class="wpvivid-nav-bar wpvivid-clear-float">
                ...
                <span>
                    <code>
                        <?php
                        echo esc_html(WPvivid_Custom_Interface_addon::wpvivid_get_local_backup_abspath());
                        ?>
                    </code>
                </span>
