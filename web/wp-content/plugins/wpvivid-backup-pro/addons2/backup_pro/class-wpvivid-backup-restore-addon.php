...
class WPvivid_BackupList_addon
{
    ...
    public function __construct()
    {
        ...
        add_action('wpvivid_restore_analyze_error', array($this, 'restore_analyze_error'), 10, 2);
    }
    ...
    public function restore_analyze_error($error, $context = array())
    {
        if ($this->is_unknown_collation_error($error)) {
            $this->mark_restore_error('unknown_collation', $error, $context);
            return;
        }
    }

    private function is_unknown_collation_error($error)
    {
        if (empty($error)) {
            return false;
        }
        ...
        return strpos($error, 'unknown collation') !== false
            || strpos($error, 'general error: 1273') !== false;
    }

    private function mark_restore_error($type, $message, $context = array())
    {
        $restore_task=get_option('wpvivid_restore_task',array());

        if (!is_array($restore_task)) {
            $restore_task = array();
        }

        $restore_task['restore_error_info'] = array(
            'type'    => $type,
            'message' => $message,
            'context' => $context,
        );
    ...
    public function get_dashboard_menu($submenus,$parent_slug)
    {
        ...
        if($display)
        {
            ...
            $submenu['page_title'] = apply_filters('wpvivid_white_label_display', __('Backup Manager', 'wpvivid-backup-pro'));
            $submenu['menu_title'] = __('Backup Manager', 'wpvivid-backup-pro');
    ...
    public function get_toolbar_menus($toolbar_menus)
    {
        ...
        if($display) {
            ...
            $menu['title'] = __('Backup Manager', 'wpvivid-backup-pro');
                            ...
                            <div class="wpvivid-backup">
                                <div class="wpvivid-welcome-bar wpvivid-clear-float">
                                    <div class="wpvivid-welcome-bar-left">
                                        <p><span class="dashicons dashicons-update-alt wpvivid-dashicons-large wpvivid-dashicons-green"></span><span class="wpvivid-page-title"><?php esc_html_e('Backup Manager & Restoration', 'wpvivid-backup-pro'); ?></span></p>
                                        <span class="about-description"><?php esc_html_e('The page allows you to browse and manage all your backups, upload backups and restore the website from backups.', 'wpvivid-backup-pro'); ?></span>
                                    </div>
                                ...
                                <div class="wpvivid-canvas wpvivid-clear-float">
                                    ...
                                    {
                                        $first_tab=true;
                                        $tabs['all_backups']['title']=__('All Backups', 'wpvivid-backup-pro');
                                    ...
                                    {
                                        $tabs['upload_backup']['title']=__('Upload', 'wpvivid-backup-pro');
                                    ...
                                    $tabs['backup_log_list']['title']=__('Logs', 'wpvivid-backup-pro');
                                    ...
                                    $args['can_delete']=1;
                                    $args['hide']=1;
                                    $tabs['download_backup']['title']=__('Download', 'wpvivid-backup-pro');
                                    ...
                                    $args['can_delete']=1;
                                    $args['hide']=1;
                                    $tabs['restore_backup']['title']=__('Restore', 'wpvivid-backup-pro');
                                    ...
                                    $args['can_delete']=1;
                                    $args['hide']=1;
                                    $tabs['view_detail']['title']=__('Content', 'wpvivid-backup-pro');
    ...
    public function output_all_backups()
    {
        ...
        <div class="tablenav top">
                ...
                <div>
                    <span style="float: left;">
                        <select id="wpvivid_select_backup_storage">
                            <option value="all_backups" selected="selected"><?php esc_html_e('All Backups', 'wpvivid-backup-pro'); ?></option>
                            <option value="localhost"><?php esc_html_e('Localhost', 'wpvivid-backup-pro'); ?></option>
                            <option value="cloud"><?php esc_html_e('Cloud Storage', 'wpvivid-backup-pro'); ?></option>
                        </select>
                    </span>
                    ...
                    <span style="float: left;">
                        <select id="wpvivid_select_backup_folder">
                            <option value="all_backup" selected="selected">--</option>
                            <option value="Manual"><?php esc_html_e('Manual Backups', 'wpvivid-backup-pro'); ?></option>
                            <option value="Cron"><?php esc_html_e('Scheduled Backups', 'wpvivid-backup-pro'); ?></option>
                            <option id="wpvivid_uploaded_option" value="Upload"><?php esc_html_e('Uploaded Backups', 'wpvivid-backup-pro'); ?></option>
                            <option id="wpvivid_migrated_option" value="Migrate"><?php esc_html_e('Backups for Migration', 'wpvivid-backup-pro'); ?></option>
                            <option value="Rollback"><?php esc_html_e('Rollback Backups', 'wpvivid-backup-pro'); ?></option>
                            <option value="Incremental"><?php esc_html_e('Incremental', 'wpvivid-backup-pro'); ?></option>
                        </select>
                    </span>
                    <span>
                        <input id="wpvivid_get_scan_backup" type="submit" class="button action top-action" value="<?php esc_attr_e('Scan', 'wpvivid-backup-pro'); ?>">
                    </span>
            ...
            <div class="tablenav-pages">
                <input type="submit" class="button-primary" id="wpvivid_rescan_local_folder_btn" value="<?php esc_attr_e('Scan uploaded backup or received backup', 'wpvivid-backup-pro'); ?>">
            </div>
            <br class="clear">
        </div>
        ...
        <div>
            <input class="button-primary" id="wpvivid-delete-localhost-array" type="submit" value="<?php esc_attr_e('Delete the selected backups', 'wpvivid-backup-pro'); ?>">
        </div>
        ...
        <script>
            ...
            jQuery('#wpvivid_backup_list').on("click",'.backuplist-delete-backup',function()
            {
                ...
                var descript = wpvivid_ajax_object_addon.i18n.confirm_remove_backup;
            ...
            jQuery('#wpvivid-delete-localhost-array').click(function() {
                ...
                {
                    var descript = wpvivid_ajax_object_addon.i18n.confirm_remove_selected_backups;
                ...
                wpvivid_post_request_addon(ajax_data, function (data)
                {
                    ...
                    try {
                        ...
                        if(typeof jsonarray.incomplete_backup !== 'undefined' && jsonarray.incomplete_backup.length > 0)
                        {
                            ...
                            alert(wpvivid_ajax_object_addon.i18n.failed_to_scan_backup_zips.replace('%s', incomplete_count));
                        }
        ...
        if(!empty($backups['remote']))
        {
            foreach ($backups['remote'] as $id=>$backup)
            {
                ...
                if(sizeof($time_array)>4)
                    ...
                    $time=$backup['date'];
                $backup_data['create_time'] = WPvivid_Time::parse_local_datetime_to_timestamp($time, 'Y-m-d H:i');
    ...
    public function output_upload_backup()
    {
        ...
        <div class="wpvivid-one-coloum wpvivid-workflow wpvivid-clear-float" style="margin-bottom:1em;">
            ...
            <span><?php esc_html_e('Backup Directory:', 'wpvivid-backup-pro'); ?></span><span><code><?php echo esc_html(WPvivid_Custom_Interface_addon::wpvivid_get_local_backup_abspath()); ?></code></span>
            <span> | </span>
