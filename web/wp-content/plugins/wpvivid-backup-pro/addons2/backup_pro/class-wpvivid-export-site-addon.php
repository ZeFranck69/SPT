...
class WPvivid_Export_Site_Page_addon
{
    ...
    public function get_dashboard_menu($submenus,$parent_slug)
    {
        ...
        if($display)
        {
            ...
            $submenu['page_title'] = apply_filters('wpvivid_white_label_display', __('Export Site', 'wpvivid-backup-pro'));
            $submenu['menu_title'] = __('Export Site', 'wpvivid-backup-pro');
    ...
    public function get_toolbar_menus($toolbar_menus)
    {
        ...
        if($display)
        {
            ...
            $menu['title'] = __('Export Site', 'wpvivid-backup-pro');
    ...
    public function _list_tasks_addon()
    {
        ...
        foreach ($tasks as $task)
        {
            ...
            if($info['status']['str']=='ready'||$info['status']['str']=='running'||$info['status']['str']=='wait_resume'||$info['status']['str']=='no_responds')
            {
                ...
                if(isset($task['options']['backup_files']) && !empty($task['options']['backup_files']))
                {
                    $ret['running_backup_type'] = $task['options']['backup_files'];
                }
                ...
                $ret['progress_html'] = '<div class="wpvivid-one-coloum wpvivid-workflow wpvivid-clear-float">
                                            <span><span class="wpvivid-backup-percent-progress">'.$info['task_info']['backup_percent'].'</span> '.esc_html__('Completed', 'wpvivid-backup-pro').'</span><br>
                                            ...
                                            <p>
                                                <span class="dashicons dashicons-admin-page wpvivid-dashicons-green"></span><span>'.esc_html__('Total Size:', 'wpvivid-backup-pro').'</span><span>'.$info['task_info']['total'].'</span>
                                                <span class="dashicons dashicons-upload wpvivid-dashicons-blue"></span><span>'.esc_html__('Uploaded:', 'wpvivid-backup-pro').'</span><span>'.$info['task_info']['upload'].'</span>
                                                <span class="dashicons dashicons-plugins-checked wpvivid-dashicons-green"></span><span>'.esc_html__('Speed:', 'wpvivid-backup-pro').'</span><span>'.$info['task_info']['speed'].'</span>
                                                <span class="dashicons dashicons-networking wpvivid-dashicons-green"></span><span>'.esc_html__('Network Connection:', 'wpvivid-backup-pro').'</span><span>'.$info['task_info']['network_connection'].'</span>
                                            </p>
                                            <p><span class="dashicons dashicons-welcome-write-blog wpvivid-dashicons-grey"></span><span>'.esc_html__('Action:', 'wpvivid-backup-pro').'</span><span id="wpvivid_current_doing">'.$info['task_info']['descript'].'</span></p>
                                            <div><input class="button-primary" id="wpvivid_backup_cancel_btn" type="submit" value="'.esc_attr__('Cancel', 'wpvivid-backup-pro').'" style="'.$info['task_info']['css_btn_cancel'].'"></div>
                                        </div>';
            }
        }
        ...
        if(!empty($finished_tasks))
        {
            ...
            {
                $notice_msg = esc_html__('Transfer succeeded. Please scan the backup list on the destination site to display the backup, then restore the backup.', 'wpvivid-backup-pro');
            ...
            if($backup_success_count>0)
            {
                ...
                $notice_msg = sprintf(__( '%s backup task(s) finished. Please switch to %s page to check the details.', 'wpvivid-backup-pro' ), number_format_i18n($backup_success_count), '<a href="'.esc_url($log_url).'">'.esc_html__('Log', 'wpvivid-backup-pro').'</a>');
            ...
            {
                $admin_url = apply_filters('wpvivid_get_admin_url', '');
                $notice_msg = sprintf(__( '%s backup task(s) failed. Please switch to %s page to send us the debug information.', 'wpvivid-backup-pro' ), number_format_i18n($backup_failed_count), '<a href="'.esc_url($admin_url.'admin.php?page='.apply_filters('wpvivid_white_label_plugin_name', 'wpvivid-debug')).'">'.esc_html__('Website Info', 'wpvivid-backup-pro').'</a>');
    ...
    public function export_site_content_addon($html, $type_name)
    {
        ...
        <label style="padding-right:2em;">
            <input type="radio" option="<?php echo esc_attr($type_name); ?>" name="<?php echo esc_attr($type_name); ?>" value="files+db" checked="checked">
            <span><?php esc_html_e('WordPress Files + Database', 'wpvivid-backup-pro'); ?></span>
        </label>
        <label style="padding-right:2em;">
            <input type="radio" option="<?php echo esc_attr($type_name); ?>" name="<?php echo esc_attr($type_name); ?>" value="db">
            <span><?php esc_html_e('Database', 'wpvivid-backup-pro'); ?></span>
        </label>
        <label style="padding-right:2em;">
            <input type="radio" option="<?php echo esc_attr($type_name); ?>" name="<?php echo esc_attr($type_name); ?>" value="files">
            <span><?php esc_html_e('WordPress Files', 'wpvivid-backup-pro'); ?></span>
        </label>
        <?php
        if(is_multisite())
        {
            ?>
            <label style="padding-right:2em;">
                <input type="radio" option="<?php echo esc_attr($type_name); ?>" name="<?php echo esc_attr($type_name); ?>" value="mu">
        ...
        ?>
        <label style="padding-right:2em;">
            <input type="radio" option="<?php echo esc_attr($type_name); ?>" name="<?php echo esc_attr($type_name); ?>" value="custom">
            <span><?php esc_html_e('Custom content', 'wpvivid-backup-pro'); ?></span>
        </label>
        <?php
        $html .= ob_get_clean();
        return $html;
    ...
    public function wpvivid_export_site_migration_addon()
    {
        ...
        if(empty($options))
        {
            ...
            <div class="wpvivid-one-coloum wpvivid-workflow" style="margin-top:1em;">
                <span>
                    <h2><?php esc_html_e('Step 1: Paste the key below:', 'wpvivid-backup-pro'); ?>
                        <span class="dashicons dashicons-editor-help wpvivid-dashicons-editor-help wpvivid-tooltip">
                            <div class="wpvivid-bottom">
                                <h3><?php esc_html_e('How to get a site key?', 'wpvivid-backup-pro'); ?></h3>
                                <!-- The content you need -->
                                <p><?php esc_html_e('1. Go to the destination site > WPvivid Plugin > Auto-Migration tab > Generate A Key sub-tab.', 'wpvivid-backup-pro'); ?></p>
                                <p><?php esc_html_e('2. Generate a key by clicking Generate button and copy it.', 'wpvivid-backup-pro'); ?></p>
                                <p><?php esc_html_e('3. Go back to this page and paste the key into the field below and click Save button.', 'wpvivid-backup-pro'); ?></p>
                ...
                <input class="button-primary" id="wpvivid_save_url_button" type="submit" value="<?php esc_attr_e('Save', 'wpvivid-backup-pro'); ?>" onclick="wpvivid_click_save_site_url();">
                ...
                <div>
                    <span>
                        ...
                        echo wp_kses(
                            sprintf(
                                __('Tips: Some web hosts may restrict the connection between the two sites, so you may get a 403 error or unstable connection issue when performing auto migration. In that case, it is recommended to use %s to migrate.', 'wpvivid-backup-pro'),
                                '<a href="https://docs.wpvivid.com/custom-migration-overview.html" target="_blank">' .
                                esc_html__("the 'manual transfer' or 'migrate via remote storage' option", 'wpvivid-backup-pro') .
                                '</a>'
                            ),
                            array(
                                'a'=>array(
        ...
        else
        {
            ...
            foreach ($options as $key => $value)
            {
                ...
                if ($expires != 0 && time() > $expires) {
                    $key_status='<span>'.esc_html__('Error: The key has expired. Please delete it first and paste a new one.', 'wpvivid-backup-pro').'</span>';
                }
                else{
                    $time_diff = $expires - time();
                    $key_status = '<p><span>'.esc_html__('The key will expire in:', 'wpvivid-backup-pro').' </span><span>'.WPvivid_Time::format_local("H:i:s",$time_diff).'</span></p>
                                   <p><span>'.esc_html__('Connection Status:', 'wpvivid-backup-pro').'</span><span class="wpvivid-rectangle wpvivid-green">OK</span></p>
                                   <p><span>'.
                                        sprintf(
                                            esc_html__('Now you can transfer the site %1$s to the site %2$s', 'wpvivid-backup-pro'),
            ...
            <div id="wpvivid_migration_export_site_success_notice" style="display: none;">
                <div class="wpvivid-v2-export-container">
                    <h1 class="wpvivid-v2-export-title">
                        🎉 <?php esc_html_e('Congratulations! Your site has been exported successfully', 'wpvivid-backup-pro'); ?>
                    </h1>

                    <div class="wpvivid-v2-export-message">
                        <p>
                            <span class="dashicons dashicons-lightbulb wpvivid-v2-export-icon"></span>
                            <strong><?php esc_html_e('The backup has been sent to your target WordPress site, you can import it on target site.', 'wpvivid-backup-pro'); ?></strong>
                        </p>
                    </div>

                    <div class="wpvivid-v2-export-action">
                        <span class="wpvivid-btn-primary wpvivid-hide-migration-export-site-notice"><?php esc_html_e('I got it', 'wpvivid-backup-pro'); ?></span>
                    </div>
                </div>
            </div>
            ...
            <div class="wpvivid-one-coloum wpvivid-workflow" style="margin-bottom: 10px;">
                ...
                <span><?php esc_html_e('Key:', 'wpvivid-backup-pro'); ?></span>
                <input type="text" id="wpvivid_send_remote_site_url_text" value="<?php echo $token; ?>" readonly="readonly">
                <input class="button-primary" id="wpvivid_delete_key_button" type="submit" value="<?php esc_attr_e('Delete', 'wpvivid-backup-pro'); ?>" onclick="wpvivid_click_delete_transfer_key();">
                <p>
                    <?php echo $key_status; ?>
                </p>
            </div>
                ...
                <div>
                    <div>
                        <p>
                            ...
                            <span><?php esc_html_e('The backup will be sent to', 'wpvivid-backup-pro'); ?> </span><span><?php printf(esc_html__('your website %s directly', 'wpvivid-backup-pro'), esc_url($target_dir)); ?></span>
                        </p>
                    </div>
                    ...
                    <div class="wpvivid-backup-custom-content">
                        ...
                        if(is_multisite())
                        {
                            ...
                            <div id="wpvivid_custom_migration_export_site_mu_single_site_list" style="display: none;">
                                <p><?php esc_html_e('Choose the childsite you want to backup', 'wpvivid-backup-pro'); ?></p>
                                <p>
                                    <span style="padding-right:0.2em;">
                                        <input type="search" style="margin-bottom: 4px; width:300px;" class="wpvivid-mu-single-site-search-input" placeholder="<?php esc_attr_e('Enter title, URL or description', 'wpvivid-backup-pro'); ?>" name="s" value="">
                                    </span>
                                    <span><input type="submit" class="button wpvivid-mu-single-search-submit" value="<?php esc_attr_e('Search', 'wpvivid-backup-pro'); ?>"></span>
                                </p>
                    ...
                    <div>
                        <p>
                            ...
                            ?>
                            <span><strong><?php esc_html_e('Comment the backup', 'wpvivid-backup-pro'); ?></strong><?php esc_html_e('(optional):', 'wpvivid-backup-pro'); ?> </span><input type="text" option="migration_export_site" name="backup_prefix" id="wpvivid_set_migration_export_site_prefix" value="<?php echo esc_attr($prefix); ?>" onkeyup="value=value.replace(/[^a-zA-Z0-9._]/g,'')" onpaste="value=value.replace(/[^\a-\z\A-\Z0-9]/g,'')" placeholder="<?php echo esc_attr($prefix); ?>">
                        </p>
                    </div>

                    <div style="margin-bottom:-1em;border-top:1px solid #f1f1f1;padding-top:1em;">
                        <input type="submit" class="button-primary" id="wpvivid_migration_export_site" value="<?php esc_attr_e('Export Now', 'wpvivid-backup-pro'); ?>" style="width: 200px; font-size: 20px; margin-bottom: 10px; pointer-events: auto; opacity: 1;" >
                    </div>
                    <div class="wpvivid-element-space-bottom" style="text-align: left; display: none;">
                        <label class="wpvivid-checkbox">
                            <span><?php esc_html_e('Marking this backup can only be deleted manually', 'wpvivid-backup-pro'); ?></span>
    ...
    public function test_connect_site()
    {
        if(isset($_POST['url']))
        {
            ...
            if (filter_var($url, FILTER_VALIDATE_URL) === FALSE)
            {
                ...
                $ret['error']=__('The key is invalid.', 'wpvivid-backup-pro');
            ...
            {
                $ret['result']=WPVIVID_FAILED;
                $ret['error']=__('The key generated by this site cannot be added into this site.', 'wpvivid-backup-pro');
                echo json_encode($ret);
                die();
            }
            ...
            if ($expires != 0 && time() > $expires) {
                $ret['result'] = 'failed';
                $ret['error'] = __('The key has expired.', 'wpvivid-backup-pro');
                echo json_encode($ret);
                die();
            }
            ...
            {
                $ret['result'] = 'failed';
                $ret['error'] = __('Data encryption failed.', 'wpvivid-backup-pro');
                echo json_encode($ret);
                die();
            }
    ...
    public function get_need_download_files()
    {
        try
        {
            ...
            {
                $ret['result']='failed';
                $ret['error']=__('Retrieving the backup(s) information failed while deleting the selected backup(s). Please try again later.', 'wpvivid-backup-pro');
                echo json_encode($ret);
                die();
            }
    ...
    public function init_page()
    {
                            ...
                            <div class="wpvivid-backup">
                                <div class="wpvivid-welcome-bar wpvivid-clear-float">
                                    <div class="wpvivid-welcome-bar-left">
                                        <p><span class="dashicons dashicons-migrate wpvivid-dashicons-large wpvivid-dashicons-blue"></span><span class="wpvivid-page-title"><?php esc_html_e('Export Site', 'wpvivid-backup-pro'); ?></span></p>
                                        <span class="about-description"><?php esc_html_e('Export the site to localhost(web server), remote storage or target site (auto-migration) for migration purpose.', 'wpvivid-backup-pro'); ?></span>
                                    </div>
                                    <div class="wpvivid-welcome-bar-right">
                                        <p></p>
                                        <div style="float:right;">
                                            <span><?php esc_html_e('Local Time', 'wpvivid-backup-pro'); ?>:</span>
                                            <span>
                                            ...
                                            <span class="dashicons dashicons-editor-help wpvivid-dashicons-editor-help wpvivid-tooltip">
                                                <div class="wpvivid-left">
                                                    <!-- The content ou need -->
                                                    <p><?php esc_html_e('Clicking the date and time will redirect you to the WordPress General Settings page where you can change your timezone settings.', 'wpvivid-backup-pro'); ?></p>
                                                    <i></i> <!-- do not delete this line -->
                                                </div>
                                            </span>
                                        </div>
                                ...
                                <div class="wpvivid-canvas wpvivid-clear-float">
                                    ...
                                    $tabs['localhost']['title']=__('Export to Localhost', 'wpvivid-backup-pro');
                                    ...
                                    $args['is_parent_tab']=0;
                                    $tabs['remote']['title']=__('Export to Remote Storage', 'wpvivid-backup-pro');
                                    ...
                                    $args['div_style']='padding-top:0;';
                                    $args['is_parent_tab']=0;
                                    $tabs['migration']['title']=__('Export to Target Site(auto-migration)', 'wpvivid-backup-pro');
        ...
        <script>
            ...
            function wpvivid_set_export_backup_type_ui_for_running_task(export_type, backup_type)
            {
                if(typeof backup_type === 'undefined' || backup_type === '')
                {
                    return;
                }
                ...
                if(export_type === 'local_export_site')
                {
                    option_name = 'local_export_site';
                    custom_box = 'wpvivid_custom_local_export_site';
                    mu_box = 'wpvivid_custom_local_export_site_mu_single_site';
                    mu_list = 'wpvivid_custom_local_export_site_mu_single_site_list';
                }
                else if(export_type === 'remote_export_site')
                {
                    option_name = 'remote_export_site';
                    custom_box = 'wpvivid_custom_remote_export_site';
                    mu_box = 'wpvivid_custom_remote_export_site_mu_single_site';
                    mu_list = 'wpvivid_custom_remote_export_site_mu_single_site_list';
                }
                else if(export_type === 'auto_migrate')
                {
                    option_name = 'migration_export_site';
                    custom_box = 'wpvivid_custom_migration_export_site';
                    mu_box = 'wpvivid_custom_migration_export_site_mu_single_site';
                    mu_list = 'wpvivid_custom_migration_export_site_mu_single_site_list';
                }
                else
                {
                    return;
                ...
                jQuery('input:radio[option=' + option_name + '][name=' + option_name + '][value="' + backup_type + '"]').prop('checked', true);

                if(backup_type === 'custom')
                {
                    jQuery('#' + custom_box).show();
                    jQuery('#' + mu_box).hide();
                    jQuery('#' + mu_list).hide();
                }
                else if(backup_type === 'mu')
                {
                    jQuery('#' + custom_box).hide();
                    jQuery('#' + mu_box).show();
                    jQuery('#' + mu_list).show();
                }
                else
            ...
            function wpvivid_list_task_data(data)
            {
                ...
                if ( typeof data.local_export_file_complete !== 'undefined' )
                {
                    ...
                    jQuery.each(data.local_export_files, function(filename, fileinfo){
                        export_site_html += ' <div backup-id="'+data.task_id+'" file-name="'+filename+'" class="wpvivid-file-item">' +
                            ...
                            '                   <span class="wpvivid-file-action wpvivid-download-export-file"><a href="#">' + wpvivid_ajax_object_addon.i18n.download + '</a></span>' +
                            '                </div>';
                    });
                ...
                if(data.running_backup_taskid!== '')
                {
                    ...
                    if(typeof data.export !== 'undefined' && typeof data.running_backup_type !== 'undefined')
                    {
                        wpvivid_set_export_backup_type_ui_for_running_task(data.export, data.running_backup_type);
                    }
    ...
    public function download_tools()
    {
        ...
        <script>
            ...
            function wpvivid_get_next_download()
            {
                ...
                {
                    alert(wpvivid_ajax_object_addon.i18n.all_files_of_the_backup_have_been_downloaded_successfully);
            ...
            function wpvivid_start_download()
            {
                ...
                }, function (code)
                {
                    ...
                    {
                        alert(wpvivid_ajax_object_addon.i18n.http_error + code);
                    }
                });
            }
                ...
                fs.root.getFile('wpvividbackups/'+file_name, {create: true, exclusive: false}, function(fileEntry)
                {
                    fileEntry.createWriter(function(fileWriter)
                    {
                        fileWriter.onerror = function(e)
                        {
                            ...
                            if(!wpvivid_download_retry())
                            {
                                alert(wpvivid_ajax_object_addon.i18n.download_failed + e.toString());
                            }
                        }
            ...
            jQuery('#wpvivid-success-content').on("click", '.wpvivid-export-site-download-all', function()
            {
                ...
                {
                    alert(wpvivid_ajax_object_addon.i18n.bulk_download_not_supported);
                    return;
                }
                    ...
                    try
                    {
                        ...
                        if (jsonarray.result === 'success')
                        {
                            ...
                            jQuery('.wpvivid-export-site-download-all').closest('div').after('<div class="" id="wpvivid_download_all_progress">' +
                                '<span><span class="wpvivid-backup-percent-progress">0%</span> <?php echo esc_js(__('Completed', 'wpvivid-backup-pro')); ?></span><br>' +
                                '<span class="wpvivid-span-progress">'+
                                '<span class="wpvivid-span-processed-progress wpvivid-span-download-processed-percent-progress"></span>'+
                                '</span>'+
                                '<p>' +
                                '<span class="wpvivid-span-download-file-name"><?php echo esc_js(__('Preparing...', 'wpvivid-backup-pro')); ?></span>' +
                                '</p>' +
                                '</div>');
    ...
    public function add_progress($type)
    {
        ...
        ?>
        <div class="wpvivid-export-site-progress" id="<?php echo esc_attr($progress_id); ?>" style="margin-bottom: 1em; display: none;">
            <div class="wpvivid-one-coloum wpvivid-workflow wpvivid-clear-float">

                <span><span class="wpvivid-backup-percent-progress">53%</span> <?php esc_html_e('Completed', 'wpvivid-backup-pro'); ?></span><br>
                <span class="wpvivid-span-progress">
                ...
                <div class="wpvivid-status-row wpvivid-margin-bottom-1rem">
                    ...
                    <div class="wpvivid-status-left">
                        <div class="wpvivid-status-info">
                            <span class="wpvivid-status-item">
                                ...
                                <span class="label"><?php esc_html_e('Total Size', 'wpvivid-backup-pro'); ?>:</span>
                                <span class="value"><?php esc_html_e('N/A', 'wpvivid-backup-pro'); ?></span>
                            </span>

                            <span class="wpvivid-status-item">
                                <span class="dashicons dashicons-upload wpvivid-dashicons-blue"></span>
                                <span class="label"><?php esc_html_e('Uploaded', 'wpvivid-backup-pro'); ?>:</span>
                                <span class="value"><?php esc_html_e('N/A', 'wpvivid-backup-pro'); ?></span>
                            </span>

                            <span class="wpvivid-status-item">
                                <span class="dashicons dashicons-plugins-checked wpvivid-dashicons-green"></span>
                                <span class="label"><?php esc_html_e('Speed', 'wpvivid-backup-pro'); ?>:</span>
                                <span class="value"><?php esc_html_e('N/A', 'wpvivid-backup-pro'); ?></span>
                            </span>

                            <span class="wpvivid-status-item">
                                <span class="dashicons dashicons-networking wpvivid-dashicons-green"></span>
                                <span class="label"><?php esc_html_e('Network Connection', 'wpvivid-backup-pro'); ?>:</span>
                                <span class="value ok"><?php esc_html_e('OK', 'wpvivid-backup-pro'); ?></span>
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="wpvivid-status-right">
                        <div class="wpvivid-log-block">
                            <div class="wpvivid-log-title"><a href=""><?php esc_html_e('Backup Log', 'wpvivid-backup-pro'); ?></a></div>
                            <div class="wpvivid-log-content"></div>
                        </div>
                    </div>
                </div>

                <div><input class="button-primary" id="wpvivid_backup_cancel_btn" type="submit" value="<?php esc_attr_e('Cancel', 'wpvivid-backup-pro'); ?>"></div>
            </div>
        </div>
        <div style="clear: both;"></div>
        <script>
    ...
    public function output_export_to_localhost()
    {
        ...
        <div class="wpvivid-one-coloum wpvivid-workflow wpvivid-clear-float" style="margin-bottom: 10px;">
            ...
            <span><?php esc_html_e('Export the site to zip file(s). You can download them to your computer.', 'wpvivid-backup-pro'); ?></span>
        </div>
            ...
            <div class="wpvivid-one-coloum wpvivid-backup-success" style="margin-bottom:1rem;">
                ...
                <div class="wpvivid-success-header">
                    <div class="wpvivid-success-title">
                        🎉 <?php esc_html_e('Congratulations, the export succeeded!', 'wpvivid-backup-pro'); ?>
                    </div>
                    <div class="wpvivid-header-buttons">
                        <button class="wpvivid-toggle-btn" id="wpvivid-toggle-content">
                            <?php esc_html_e('Download Now', 'wpvivid-backup-pro'); ?>
                        </button>
                ...
                <div class="wpvivid-success-content" id="wpvivid-success-content" style="display: none;">
                    <p class="wpvivid-success-subtitle">
                        <span class="dashicons dashicons-lightbulb wpvivid-dashicons-orange"></span>
                        <?php esc_html_e('You can download all parts of the backup now, or later on', 'wpvivid-backup-pro'); ?>
                        <a href="<?php echo apply_filters('wpvivid_white_label_page_redirect', 'admin.php?page=wpvivid-backup-and-restore', 'wpvivid-backup-and-restore').'&localhost_backuplist'; ?>"><?php esc_html_e('Backup Manager Page', 'wpvivid-backup-pro'); ?></a>.
                    </p>
                    ...
                    <div class="wpvivid-success-footer">
                        <label><input type="checkbox" name="wpvivid_hide_download_part"> <?php esc_html_e('Don\'t show again', 'wpvivid-backup-pro'); ?></label>
                        <button class="wpvivid-btn-primary wpvivid-export-site-download-all"><?php esc_html_e('Download All Parts', 'wpvivid-backup-pro'); ?></button>
                        <button class="wpvivid-close-btn" id="wpvivid-close-block">
                            <?php esc_html_e('Close', 'wpvivid-backup-pro'); ?>
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div>
                    <p>
                        ...
                        ?>
                        <span><strong><?php esc_html_e('Comment the backup', 'wpvivid-backup-pro'); ?></strong><?php esc_html_e('(optional)', 'wpvivid-backup-pro'); ?>: </span><input type="text" option="local_export_site" name="backup_prefix" id="wpvivid_set_local_export_site_prefix" value="<?php echo esc_attr($prefix); ?>" onkeyup="value=value.replace(/[^a-zA-Z0-9._]/g,'')" onpaste="value=value.replace(/[^\a-\z\A-\Z0-9]/g,'')" placeholder="<?php echo esc_attr($prefix); ?>">
                    </p>
                </div>

                <div style="margin-bottom:-1em;border-top:1px solid #f1f1f1;padding-top:1em;">
                    <input type="submit" class="button-primary" id="wpvivid_local_export_site" value="<?php esc_attr_e('Export Now', 'wpvivid-backup-pro'); ?>" style="width: 200px; font-size: 20px; margin-bottom: 10px; pointer-events: auto; opacity: 1;" >
                </div>
                <div class="wpvivid-element-space-bottom" style="text-align: left; display: none;">
                    <label class="wpvivid-checkbox">
                        <span><?php esc_html_e('Marking this backup can only be deleted manually', 'wpvivid-backup-pro'); ?></span>
            ...
            function wpvivid_check_backup_option_avail(type)
            {
                ...
                if(jQuery('#'+parent_id).find('.wpvivid-custom-database-part').prop('checked')){
                    ...
                    if(jQuery('#'+parent_id).find('.wpvivid-custom-database-check').prop('checked'))
                    {
                        ...
                        {
                            check_status = false;
                            alert(wpvivid_ajax_object_addon.i18n.select_wp_database_table_required);
                            return check_status;
                        }
                    }
                    if(jQuery('#'+parent_id).find('.wpvivid-custom-additional-database-check').prop('checked'))
                    {
                        ...
                        {
                            check_status = false;
                            alert(wpvivid_ajax_object_addon.i18n.please_select_at_least_one_additional_database_to_back_up);
                            return check_status;
                        }
                    }
                    if(!has_db_item){
                        check_status = false;
                        alert(wpvivid_ajax_object_addon.i18n.select_database_backup_option_required);
                        return check_status;
                    }
                }
                if(jQuery('#'+parent_id).find('.wpvivid-custom-file-part').prop('checked')){
                    ...
                    if(jQuery('#'+parent_id).find('.wpvivid-custom-additional-folder-check').prop('checked')){
                        ...
                        if(!has_additional_folder){
                            check_status = false;
                            alert(wpvivid_ajax_object_addon.i18n.select_additional_file_or_folder_required);
                            return check_status;
                        }
                    }
                    if(!has_file_item){
                        check_status = false;
                        alert(wpvivid_ajax_object_addon.i18n.select_files_backup_option_required);
                        return check_status;
                    }
                }
            ...
            <div id="wpvivid_remote_export_site_success_notice" style="display: none;">
                ...
                <div class="wpvivid-v2-export-container">
                    ...
                    <div class="wpvivid-v2-export-message">
                        <p>
                            <span class="dashicons dashicons-lightbulb wpvivid-v2-export-icon"></span>
                            <strong><?php esc_html_e('The backup has been sent to your remote storage.', 'wpvivid-backup-pro'); ?></strong>
                        </p>
                        <p>
                            <strong>
                                <?php esc_html_e('You can import it on another WordPress site to complete migration', 'wpvivid-backup-pro'); ?> —
                                <a href="https://docs.wpvivid.com/wpvivid-backup-pro-migrate-wordpress-site-via-remote-storage.html" class="wpvivid-v2-export-link" target='_blank'><?php esc_html_e('learn more...', 'wpvivid-backup-pro'); ?></a>
                            </strong>
                        </p>
                        <p>
                            <strong>
                                <?php esc_html_e('You can download the backup anytime on', 'wpvivid-backup-pro'); ?>
                                <a href="<?php echo apply_filters('wpvivid_white_label_page_redirect', 'admin.php?page=wpvivid-backup-and-restore', 'wpvivid-backup-and-restore').'&remote_backuplist'; ?>" class="wpvivid-v2-export-link"><?php esc_html_e('Backup Manager Page', 'wpvivid-backup-pro'); ?></a>.
                            </strong>
                        </p>
                    </div>

                    <div class="wpvivid-v2-export-action">
                        <span class="wpvivid-btn-primary wpvivid-hide-remote-export-site-notice"><?php esc_html_e('I got it', 'wpvivid-backup-pro'); ?></span>
                    </div>
                </div>
            </div>
            ...
            <div class="wpvivid-one-coloum wpvivid-workflow">
                <div>
                    <p>
                        ...
                        <span><?php esc_html_e('The backup will be sent to', 'wpvivid-backup-pro'); ?> </span>
                        <span>
                            <select id="wpvivid_select_export_remote_storage">
                                ...
                                foreach ($remoteslist as $key => $value)
                                {
                                    ...
                                    $type = apply_filters('wpvivid_storage_provider_tran', $value['type']);
                ...
                <div>
                    <p>
                        ...
                        ?>
                        <span><strong><?php esc_html_e('Comment the backup', 'wpvivid-backup-pro'); ?></strong><?php esc_html_e('(optional)', 'wpvivid-backup-pro'); ?>: </span><input type="text" option="remote_export_site" name="backup_prefix" id="wpvivid_set_remote_export_site_prefix" value="<?php echo esc_attr($prefix); ?>" onkeyup="value=value.replace(/[^a-zA-Z0-9._]/g,'')" onpaste="value=value.replace(/[^\a-\z\A-\Z0-9]/g,'')" placeholder="<?php echo esc_attr($prefix); ?>">
                    </p>
                </div>

                <div style="margin-bottom:-1em;border-top:1px solid #f1f1f1;padding-top:1em;">
                    <input type="submit" class="button-primary" id="wpvivid_remote_export_site" value="<?php esc_attr_e('Export Now', 'wpvivid-backup-pro'); ?>" style="width: 200px; font-size: 20px; margin-bottom: 10px; pointer-events: auto; opacity: 1;" >
                </div>
                <div class="wpvivid-element-space-bottom" style="text-align: left; display: none;">
                    <label class="wpvivid-checkbox">
                        <span><?php esc_html_e('Marking this backup can only be deleted manually', 'wpvivid-backup-pro'); ?></span>
        ...
        else
        {
            ...
            <div>
                <div class="wpvivid-v2-alert">
                    ...
                    <?php esc_html_e('There is no available remote storage added. Please set an available account on', 'wpvivid-backup-pro'); ?>
                    <a href="<?php echo 'admin.php?page='.strtolower(sprintf('%s-remote', apply_filters('wpvivid_white_label_slug', 'wpvivid')));?>" style="cursor: pointer;"><?php esc_html_e('Cloud Storage', 'wpvivid-backup-pro'); ?></a> <?php esc_html_e('page.', 'wpvivid-backup-pro'); ?>
                </div>
            </div>
            <?php
        }
    ...
    public function output_export_to_migration()
    {
        ...
        <script>
            ...
            function wpvivid_check_key(value)
            {
                ...
                {
                    alert(wpvivid_ajax_object_addon.i18n.key_generated_by_this_site_cannot_be_added);
