...
class WPvivid_Mysqldump_Addon
{
    ...
    public function list_tables()
    {
        ...
        if(isset($this->dumpSettings['prefix'])&&!empty($this->dumpSettings['prefix']))
        {
            ...
            {
                global $wpdb;
                if(isset($this->dumpSettings['mu_single_site']) && $this->dumpSettings['mu_single_site'] === true)
                {
                    if ($this->dumpSettings['prefix'] === $wpdb->base_prefix)
                    {
                        $exclude = array('/^' . preg_quote($wpdb->base_prefix, '/') . '\d+_/i');
                    }
                    else
                    {
    ...
    private function listValues($tableName,$index)
    {
        ...
        if ($b_options)
        {
            $exclude_options = apply_filters('wpvivid_exclude_options_from_database_backup', array('wpvivid_task_list'));

            if (!is_array($exclude_options))
            {
                $exclude_options = array('wpvivid_task_list');
            }

            foreach ($exclude_options as $key => $option_name)
            {
                $exclude_options[$key] = $this->typeAdapter->quote($option_name);
            }
        }
        ...
        {
            $res = $this->query("SELECT MAX(`{$primary_key}`) AS max_id FROM `{$tableName}`");
            $max_id = null;
            foreach ($res as $row) {
                $max_id = isset($row['max_id']) ? $row['max_id'] : null;
            }
            $this->typeAdapter->closeCursor($res);
        ...
        while($sum > $start)
        {
            if ($use_offset) {
                ...
                $query = $stmt . $where . " `$primary_key` > $start ORDER BY `$primary_key` ASC LIMIT $limit_count";
            }
            ...
            foreach ($resultSet as $row)
            {
                ...
                foreach($vals as $key => $value)
                {
                    ...
                    if($b_options)
                    {
                        if (in_array($value, $exclude_options, true))
                        {
                            $skip=true;
                        }
                    }
                ...
                if (!$use_offset) {
                    $start = max($start, $row[$primary_key]);
                }
    ...
    private function find_primary_key($tableName)
    {
        try
        {
            ...
            {
                $key_name = isset($row['Key_name']) ? $row['Key_name'] : (isset($row['key_name']) ? $row['key_name'] : '');
                $col_name = isset($row['Column_name']) ? $row['Column_name'] : (isset($row['column_name']) ? $row['column_name'] : '');

                if (strcasecmp($key_name, 'PRIMARY') === 0 && !empty($col_name))
                {
            ...
            $query = "SHOW FIELDS FROM `".$tableName."` LIKE '".$primary_key."'";
            $colInfo = $this->query($query);
            if (empty($colInfo))
            {
                return false;
            }
            ...
            if (is_array($colInfo)) {
                $rows2 = $colInfo;
            } elseif (is_object($colInfo) && method_exists($colInfo, 'fetchAll')) {
                $rows2 = $colInfo->fetchAll(\PDO::FETCH_ASSOC);
                if (method_exists($colInfo, 'closeCursor')) {
                    $colInfo->closeCursor();
                } elseif (is_object($this->typeAdapter) && method_exists($this->typeAdapter, 'closeCursor')) {
                    $this->typeAdapter->closeCursor($colInfo);
                }
            } else {
                foreach ((array)$colInfo as $r) {
                    $rows2[] = (array)$r;
                }
            }
            ...
            $typeRaw = isset($rows2[0]['Type']) ? $rows2[0]['Type']
                : (isset($rows2[0]['type']) ? $rows2[0]['type'] : null);
            if ($typeRaw === null) {
                return false;
            }
            $type = strtolower($typeRaw);

            $numeric_types = ['int', 'integer', 'bigint', 'smallint', 'mediumint', 'tinyint'];
            $is_numeric = false;
            foreach ($numeric_types as $t)
            {
