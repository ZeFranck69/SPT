...
class WPvivid_Schedule_List extends WP_List_Table
{
    ...
    {
        $columns = array();
        $columns['wpvivid_backup_type'] = __( 'Backup Type', 'wpvivid-backup-pro'  );
        $columns['wpvivid_backup_cycles'] = __( 'Backup Cycles', 'wpvivid-backup-pro'  );
        $columns['wpvivid_last_backup'] = __( 'Last Backup', 'wpvivid-backup-pro'  );
        $columns['wpvivid_next_backup'] = __( 'Next Backup', 'wpvivid-backup-pro'  );
        $columns['wpvivid_storage'] = __( 'Storage', 'wpvivid-backup-pro'  );
        $columns['wpvivid_on_off_control'] = __( 'On/off', 'wpvivid-backup-pro'  );
        $columns['wpvivid_actions'] = __( 'Actions', 'wpvivid-backup-pro'  );
        return $columns;
    }
    ...
    public function display_rows_or_placeholder() {
        if ( $this->has_items() ) {
            ...
            esc_html_e('There is no schedule created yet.', 'wpvivid-backup-pro');
            echo '</td></tr>';
        }
    }
...
class WPvivid_Schedule_Display_Addon
{
    ...
    public function get_dashboard_menu($submenus,$parent_slug)
    {
        ...
        if($display)
        {
            ...
            $submenu['page_title'] = apply_filters('wpvivid_white_label_display', __('Backup Schedule', 'wpvivid-backup-pro'));
            $submenu['menu_title'] = __('Backup Schedule', 'wpvivid-backup-pro');
    ...
    public function get_toolbar_menus($toolbar_menus)
    {
        ...
        if($display) {
            ...
            $menu['title'] = __('Backup Schedule', 'wpvivid-backup-pro');
    ...
    public function wpvivid_schedulepage_add_backup_type($html, $type)
    {
        ...
        <label style="padding-right:2em;">
            <input type="radio" option="<?php echo esc_attr($type); ?>" name="<?php echo esc_attr($type); ?>_backup_type" value="files+db" checked="checked">
    ...
    public function check_schedule_option($data)
    {
        ...
        if(!empty($data['save_local_remote']))
        {
            if($data['save_local_remote'] == 'remote')
            {
                ...
                {
                    $ret['result']=WPVIVID_PRO_FAILED;
                    $ret['error']=__('There is no default remote storage configured. Please set it up first.', 'wpvivid-backup-pro');
                    $ret['notice'] = apply_filters('wpvivid_set_schedule_notice', false, $ret['error']);
                    return $ret;
                }
                else
        ...
        {
            $ret['error']=__('Not found select backup type. Please set it up first.', 'wpvivid-backup-pro');
            return $ret;
        }*/
    ...
    public function add_schedule($schedule)
    {
        ...
        {
            $ret['result']='failed';
            $ret['error']=__('Failed to create a schedule. Please try again later.', 'wpvivid-backup-pro');
            $ret['notice'] = apply_filters('wpvivid_set_schedule_notice', false, $ret['error']);
            return $ret;
        }
        ...
        $ret['result']='success';
        $success_msg = __('You have successfully added a schedule.','wpvivid-backup-pro');
            ...
            if(isset($_POST['schedule'])&&!empty($_POST['schedule']))
            {
                ...
                if(!empty($schedules_list)){
                    $ret['create_part'] = '<span>
                                            ...
                                            <span>
                                                <input id="wpvivid_click_save_schedule_changed" type="submit" class="button action" value="'.esc_attr__('Apply', 'wpvivid-backup-pro').'" />
                                            </span>
                                            <span id="wpvivid_create_schedule_btn" style="padding:1em 1em 1em 0;">or, <a href="#">create a new schedule</a></span>';
                }
                else{
                    $ret['create_part'] = '<span><input class="button-primary" id="wpvivid_create_schedule_btn" type="submit" value="'.esc_attr__('Create a new schedule', 'wpvivid-backup-pro').'" /></span>';
                }
    ...
    public function run_schedule()
    {
        ...
        try{
                ...
                foreach ( (array) $cronhooks as $hook => $args )
                {
                    if($hook === $schedule_id)
                    {
                        foreach($args as $sig => $arg)
                        {
                            ...
                            {
                                $ret['result']='failed';
                                $ret['error']=__('Failed to add the schedule to the cron queue. Please try it again.','wpvivid-backup-pro');
                            ...
                            {
                                $ret['result']='success';
                                $success_msg = __('The schedule has been triggered successfully.','wpvivid-backup-pro');
                                $ret['notice'] = apply_filters('wpvivid_set_schedule_notice', true, $success_msg);
                            }
                            else
                            {
                                $ret['result']='failed';
                                $ret['error']=__('Failed to run the schedule. Please try it again.','wpvivid-backup-pro');
                                $ret['error'] = apply_filters('wpvivid_set_schedule_notice', false, $ret['error']);
                            }
                            echo json_encode($ret);
                            die();
            ...
            $ret['result']='failed';
            $ret['error']=__('The schedule does not exist in the cron queue. Please try to update the backup schedule then try it again.','wpvivid-backup-pro');
            $ret['error'] = apply_filters('wpvivid_set_schedule_notice', false, $ret['error']);
            echo json_encode($ret);
        }
        catch (Exception $error) {
    ...
    public function delete_schedule()
    {
        ...
        try {
            ...
            $ret['result'] = 'success';
            $success_msg = __('The schedule has been deleted successfully.','wpvivid-backup-pro');
            $ret['notice'] = apply_filters('wpvivid_set_schedule_notice', true, $success_msg);
            ...
            if(isset($_POST['schedule']) && !empty($_POST['schedule']) && isset($_POST['id']) && !empty($_POST['id']))
            {
                ...
                if($schedules[$schedule_id]['status'] === 'Active')
                {
                    ...
                    if(wp_schedule_event($schedules[$schedule_id]['start_time'], $schedules[$schedule_id]['type'], $schedules[$schedule_id]['id'], array($schedules[$schedule_id]['id']))===false)
                    {
                        $has_error = true;
                        $ret['result']='failed';
                        $ret['error']=__('Failed to update the schedule. Please try again later.', 'wpvivid-backup-pro');
                        $ret['notice'] = apply_filters('wpvivid_set_schedule_notice', false, $ret['error']);
                    }
                    else {
                        $has_error = false;
                ...
                if(!$has_error){
                    WPvivid_Setting::update_option('wpvivid_schedule_addon_setting',$schedules);
                    $ret['result']='success';
                    $success_msg = __('You have successfully updated the schedule.','wpvivid-backup-pro');
                    $ret['notice'] = apply_filters('wpvivid_set_schedule_notice', true, $success_msg);
                    $ret['data']=$schedules;
            ...
            if(isset($_POST['schedule_data']) && !empty($_POST['schedule_data']))
            {
                ...
                foreach ($schedule_data as $schedule_id => $schedule_status)
                {
                    ...
                    if($schedule_status === 'Active')
                    {
                        ...
                        if (wp_schedule_event($schedules[$schedule_id]['start_time'], $schedules[$schedule_id]['type'], $schedules[$schedule_id]['id'], array($schedules[$schedule_id]['id'])) === false) {
                            $ret['result'] = 'failed';
                            $ret['error'] = __('Failed to save the schedule. Please try again later.', 'wpvivid-backup-pro');
                            $ret['notice'] = apply_filters('wpvivid_set_schedule_notice', false, $ret['error']);
                            echo json_encode($ret);
                            die();
                        }
        ...
        try
        {
            if(isset($_POST['status']) && !empty($_POST['status']) && isset($_POST['id']) && !empty($_POST['id']))
            {
                ...
                if($status === 'Active')
                {
                    ...
                    {
                        $ret['result'] = 'failed';
                        $ret['error'] = __('Update scheduled tasks failed. Please try again later.', 'wpvivid-backup-pro');
                        echo json_encode($ret);
                        die();
                    }
                }
                            ...
                            <div class="wpvivid-dashboard">
                                <div class="wpvivid-welcome-bar wpvivid-clear-float">
                                    <div class="wpvivid-welcome-bar-left">
                                        <p><span class="dashicons dashicons-calendar-alt wpvivid-dashicons-large wpvivid-dashicons-green"></span><span class="wpvivid-page-title"><?php esc_html_e('Backup Schedule', 'wpvivid-backup-pro'); ?></span></p>
                                        <span class="about-description"><?php esc_html_e('The page allows you to create general or incremental backup schedules.', 'wpvivid-backup-pro'); ?></span>
                                    </div>
                                ...
                                <div class="wpvivid-canvas wpvivid-clear-float">
                                    ...
                                    $tabs['incremental_backup_schedules']['title']=__('Incremental Backup', 'wpvivid-backup-pro');
                                    ...
                                    $args['div_style']='';
                                    $args['is_parent_tab']=0;
                                    $tabs['full_backup']['title']=__('General Backup', 'wpvivid-backup-pro');
                                    ...
                                    $args['div_style']='';
                                    $args['is_parent_tab']=0;
                                    $tabs['imgoptim']['title']=__('Image Optimization', 'wpvivid-backup-pro');
                                    ...
                                    $args['div_style']='';
                                    $args['is_parent_tab']=0;
                                    $tabs['unused_image_cleaner']['title']=__('Unused Image Cleaner', 'wpvivid-backup-pro');
                                    ...
                                    $args['can_delete']=1;
                                    $args['hide']=1;
                                    $tabs['update_full_backup']['title']=__('Update Full Backup', 'wpvivid-backup-pro');
    ...
    public function output_full_backup()
    {
        ...
        <!--<div class="wpvivid-one-coloum" style="padding-top:0em;padding-left:0em;">
            <div class="wpvivid-two-col">
                <label class="wpvivid-switch">
                    <input type="checkbox" id="wpvivid_schedule_backup_switch" <?php echo esc_attr($schedule_enable_status); ?>>
                    <span class="wpvivid-slider wpvivid-round"></span>
                </label>
                <label>
        ...
        <div style="padding-bottom:1em;">
            <span><?php esc_html_e('Enable one or more backup schedules, you have 3 pre-set schedules.', 'wpvivid-backup-pro'); ?></span>
        </div>
        ...
        <div id="wpvivid_create_schedule_part" style="padding: 10px 0 10px 0;">
            <span><input class="button-primary" id="wpvivid_create_schedule_btn" type="submit" value="<?php esc_attr_e('Create a job', 'wpvivid-backup-pro'); ?>" /></span>
            <?php
            /*if(!empty($schedules_list)){
                ?>
                <span>
                ...
                ?>
                <span><input class="button-primary" id="wpvivid_create_schedule_btn" type="submit" value="'.esc_attr__('Create a new schedule', 'wpvivid-backup-pro').'" /></span>
                <?php
            }*/
            ?>
        </div>
        ...
        <div id="wpvivid_schedule_backup_deploy" style="display: none;">
            <table class="wp-list-table widefat plugin">
                ...
                <tr>
                    <th></th>
                    <th class="manage-column column-primary"><strong><?php esc_html_e('Local Time', 'wpvivid-backup-pro'); ?> </strong><a href="<?php esc_attr_e(admin_url().'options-general.php'); ?>"><?php esc_html_e('(Timezone Setting)', 'wpvivid-backup-pro'); ?></a></th>
                    <th class="manage-column column-primary"><strong><?php esc_html_e('Universal Time (UTC)', 'wpvivid-backup-pro'); ?></strong></th>
                </tr>
                </thead>
                <tbody>
                <tr>
                    <th><strong><?php esc_html_e('Current Time', 'wpvivid-backup-pro'); ?></strong></th>
                    <td>
                        <div>
                            <span><?php echo esc_html($local_time); ?></span>
                        </div>
                    </td>
                    <td>
                        <div>
                            <span><?php echo esc_html($utc_time); ?></span>
                        </div>
                    </td>
                </tr>
                <tr>
                    <th><strong><?php esc_html_e('Schedule Start Time', 'wpvivid-backup-pro'); ?></strong></th>
                    <td>
                        <span>
                            <select id="wpvivid_schedule_backup_rate_select" value="wpvivid_daily" option="schedule_backup" name="recurrence" onchange="click_select_rate('schedule_backup');">
                                ...
                                <option value="wpvivid_weekly" selected="selected"><?php esc_html_e('Weekly', 'wpvivid-backup-pro'); ?></option>
                        ...
                        <span>
                            <select id="wpvivid_schedule_backup_start_week_select" option="schedule_backup" name="week">
                                <option value="sun"><?php esc_html_e('Sunday', 'wpvivid-backup-pro'); ?></option>
                                <option value="mon" selected="selected"><?php esc_html_e('Monday', 'wpvivid-backup-pro'); ?></option>
                                <option value="tue"><?php esc_html_e('Tuesday', 'wpvivid-backup-pro'); ?></option>
                                <option value="wed"><?php esc_html_e('Wednesday', 'wpvivid-backup-pro'); ?></option>
                                <option value="thu"><?php esc_html_e('Thursday', 'wpvivid-backup-pro'); ?></option>
                                <option value="fri"><?php esc_html_e('Friday', 'wpvivid-backup-pro'); ?></option>
                                <option value="sat"><?php esc_html_e('Saturday', 'wpvivid-backup-pro'); ?></option>
                            </select>
                        </span>
                        <span>
                            <span>
                ...
                <tr>
                    <th colspan="3">
                        <small>
                            <span><?php esc_html_e('The schedule will be performed at', 'wpvivid-backup-pro'); ?> [<?php esc_html_e('(local time)', 'wpvivid-backup-pro'); ?></span><span id="wpvivid_local_time" style="margin-right: 0;">00:00</span><span>] [<?php esc_html_e('UTC', 'wpvivid-backup-pro'); ?></span><span id="wpvivid_utc_time_2" style="margin-right: 0;">00:00</span><span>] [<?php esc_html_e('Schedule Cycles', 'wpvivid-backup-pro'); ?>:</span><span id="wpvivid_schedule_backup_cycles" style="margin-right: 0;">Daily</span>]
                        </small>
                    </th>
                </tr>
                </tfoot>
            ...
            <div class="wpvivid-one-coloum wpvivid-workflow wpvivid-clear-float" style="margin-top:1em;">
                ...
                <div style="">
                    <p><span class="dashicons dashicons-backup wpvivid-dashicons-blue"></span><span><strong><?php esc_html_e('Backup Location', 'wpvivid-backup-pro'); ?></strong></span></p>
                    <div style="padding-left:2em;">
                        <label class="">
                            <input type="radio" option="schedule" name="schedule_save_local_remote" value="local" checked="checked" /><?php esc_html_e('Backup to localhost', 'wpvivid-backup-pro'); ?>
                        </label>
                        ...
                        if( apply_filters('wpvivid_current_user_can',true,'wpvivid-can-schedule-backup-remote'))
                        {
                            ?>
                            <label class="">
                                <input type="radio" option="schedule" name="schedule_save_local_remote" value="remote" /><?php esc_html_e('Backup to remote storage', 'wpvivid-backup-pro'); ?>
                            </label>
                            ...
                            <span id="wpvivid_create_schedule_backup_remote_selector_part" style="display: none;">
                                <select id="wpvivid_create_schedule_backup_remote_selector">
                                    ...
                                    foreach ($remoteslist as $key=>$remote_option)
                                    {
                                        ...
                                        ?>
                                        <option value="<?php echo esc_attr($remote_option['id']); ?>" selected="selected"><?php echo $remote_option['name']; ?></option>
                                        <?php
                                    }
                                    ?>
                                    <option value="all"><?php esc_html_e('All activated remote storage', 'wpvivid-backup-pro'); ?></option>
                                </select>
                            </span>
                            <?php
                        }
                ...
                <div style="">
                    <p><span class="dashicons dashicons-screenoptions wpvivid-dashicons-blue"></span><span><strong><?php esc_html_e('Backup Content', 'wpvivid-backup-pro'); ?></strong></span></p>
                ...
                <div>
                    <p>
                        ...
                        <span><strong><?php esc_html_e('Comment the backup', 'wpvivid-backup-pro'); ?></strong><?php esc_html_e('(optional)', 'wpvivid-backup-pro'); ?>: </span><input type="text" option="schedule" name="backup_prefix" id="wpvivid_set_schedule_prefix" value="<?php echo $default_prefix;?>" onkeyup="value=value.replace(/[^a-zA-Z0-9._]/g,'')" onpaste="value=value.replace(/[^\a-\z\A-\Z0-9]/g,'')" placeholder="<?php echo $default_prefix;?>">
                    </p>
                </div>
            </div>
            ...
            <div class="wpvivid-clear-float" style="padding-top:1em;">
                <label>
                    ...
                    <span><?php esc_html_e('Activate this backup schedule once created', 'wpvivid-backup-pro'); ?></span>
                </label>
            </div>

            <div class="wpvivid-one-coloum wpvivid-clear-float" style="padding-bottom:0;padding-left:0;">
                <input class="button-primary" id="wpvivid_btn_create_general_schedule" type="submit" value="<?php esc_attr_e('Create Now', 'wpvivid-backup-pro'); ?>" onclick="wpvivid_click_create_schedule();" />
            </div>
        </div>
        ...
        <script>
            ...
            jQuery('#wpvivid_create_schedule_part').on('click', '#wpvivid_click_save_schedule_changed', function(){
                ...
                if(wpvivid_select_schedule_status == '0'){
                    alert(wpvivid_ajax_object_addon.i18n.please_select_at_least_one_item_to_perform_this_action);
                    return;
                }
                else{
                    var json = {};
                ...
                if(use_new_custom_backup_ui == '1'){
                    ...
                    if(jQuery('input:radio[option='+option+'][name='+name+'][value=custom]').prop('checked')){
                        ...
                        if(check_status === false){
                            alert(wpvivid_ajax_object_addon.i18n.select_additional_files_option_required);
                        }
                    }
                    else{
                        check_status = true;
            ...
            function wpvivid_check_additional_db_valid(type){
                ...
                if(use_new_custom_backup_ui == '1'){
                    ...
                    if(jQuery('input:radio[option='+option+'][name='+name+'][value=custom]').prop('checked')){
                        ...
                        if(check_status === false){
                            alert(wpvivid_ajax_object_addon.i18n.select_additional_database_option_required);
                        }
                    }
                    else{
                        check_status = true;
        ...
        <div class="wpvivid-one-coloum wpvivid-workflow" style="margin-bottom:1em;">
            <table class="wp-list-table widefat plugin">
                ...
                <tr>
                    <th colspan="3">
                        <small>
                            <span><?php esc_html_e('The schedule will be performed at', 'wpvivid-backup-pro'); ?> [<?php esc_html_e('(local time)', 'wpvivid-backup-pro'); ?></span><span id="wpvivid_update_local_time" style="margin-right: 0;">00:00</span><span>] [<?php esc_html_e('UTC', 'wpvivid-backup-pro'); ?></span><span id="wpvivid_update_utc_time_2" style="margin-right: 0;">00:00</span><span>] [<?php esc_html_e('Schedule Cycles', 'wpvivid-backup-pro'); ?>:</span><span id="wpvivid_update_schedule_backup_cycles" style="margin-right: 0;">Daily</span>]
                        </small>
                    </th>
                </tr>
                </tfoot>
            ...
            <div class="wpvivid-one-coloum wpvivid-workflow wpvivid-clear-float" style="margin-top:1em;">
                ...
                <div style="">
                    ...
                    <div style="padding-left:2em;">
                        <label class="">
                            <input type="radio" option="update_schedule_backup" name="update_schedule_backup_save_local_remote" value="local" checked="checked" /><?php esc_html_e('Backup to localhost', 'wpvivid-backup-pro'); ?>
                        </label>
                        <span style="padding: 0 1em;"></span>
                        <?php
                        if( apply_filters('wpvivid_current_user_can',true,'wpvivid-can-schedule-backup-remote'))
                        {
                            ?>
                            <label class="">
                                <input type="radio" option="update_schedule_backup" name="update_schedule_backup_save_local_remote" value="remote" /><?php esc_html_e('Backup to remote storage', 'wpvivid-backup-pro'); ?>
                            </label>
                            <span style="padding: 0 0.2em;"></span>
                ...
                <div>
                    <p>
                        <span class="dashicons dashicons-welcome-write-blog wpvivid-dashicons-green" style="margin-top:0.2em;"></span>
                        <span><strong><?php esc_html_e('Comment the backup', 'wpvivid-backup-pro'); ?></strong><?php esc_html_e('(optional)', 'wpvivid-backup-pro'); ?>: </span><input type="text" option="update_schedule_backup" name="backup_prefix" id="wpvivid_set_update_schedule_prefix" value="<?php echo $default_prefix;?>" onkeyup="value=value.replace(/[^a-zA-Z0-9._]/g,'')" onpaste="value=value.replace(/[^\a-\z\A-\Z0-9]/g,'')" placeholder="<?php echo $default_prefix;?>">
                    </p>
                </div>
            </div>

            <div class="wpvivid-one-coloum wpvivid-clear-float" style="padding-bottom:0;padding-left:0;">
                <input class="button-primary" id="wpvivid_btn_update_general_schedule" type="submit" value="<?php esc_attr_e('Update Schedule', 'wpvivid-backup-pro'); ?>" onclick="wpvivid_click_update_schedule();" />
            </div>
        </div>
        <script>
            <?php
