...
class WPvivid_Incremental_Backup_list extends WP_List_Table
{
    ...
    {
        $columns = array();
        $columns['wpvivid_backup_type'] = __( 'Backup Content', 'wpvivid-backup-pro' );
        $columns['wpvivid_backup_cycles'] = __( 'Cycles', 'wpvivid-backup-pro'  );
        $columns['wpvivid_last_backup'] = __( 'Latest Backup', 'wpvivid-backup-pro'  );
...
class WPvivid_Incremental_Backup_Display_addon
{
    ...
    public function add_incremental_schedule($schedule, $is_mainwp=false)
    {
        ...
        if($enable_incremental_schedules)
        {
            ...
            if(wp_schedule_single_event($schedule_data['files_start_time'] + 600, $schedule_data['db_schedule_id'],array($schedule_data['id']))===false)
            {
                ...
                {
                    $ret['error']=__('To avoid conflict, please set the start time of Database (Full Backup) at least 21 minutes later than the start time of Files (Full Backup).', 'wpvivid-backup-pro');
                }
                else
                {
                    $ret['error']=__('Failed to create a schedule. Please try again later.', 'wpvivid-backup-pro');
        ...
        else
        {
            if(isset($schedule_backup_options['backup_select']))//
            {
                ...
                if(isset($schedule_backup_options['backup_select']['additional_db'])&&$schedule_backup_options['backup_select']['additional_db']==1)
                {
                    ...
                    {
                        $custom_options['additional_database_list']=$schedule_backup_options['additional_database_list'];
                    }
                    else
                    {
                        $custom_options['additional_database_list']=array();
                    }
                }
    ...
    public function edit_incremental_schedule_addon()
    {
        ...
        try{
            ...
            if($ret['backup_db_type'] === 'custom')
            {
                ...
                $custom_dirs = isset($schedule_data['backup_db']['custom_dirs']) && is_array($schedule_data['backup_db']['custom_dirs']) ? $schedule_data['backup_db']['custom_dirs'] : array();
                $ret['additional_database_list'] = isset($custom_dirs['additional_database_list']) && is_array($custom_dirs['additional_database_list']) ? $custom_dirs['additional_database_list'] : array();
            }
    ...
    public function output_incremental_backup()
    {
        ...
        <div class="wpvivid-one-coloum" style="padding-top:0em;padding-left:0em;">
            <div class="wpvivid-two-col">
                <label class="wpvivid-switch">
                    <input type="checkbox" id="wpvivid_incremental_backup_switch" <?php echo esc_attr($incremental_enable_status); ?>>
                    <span class="wpvivid-slider wpvivid-round"></span>
                </label>
                <label>
                    <span><?php esc_html_e('Enable Incremental Backup Schedule', 'wpvivid-backup-pro'); ?></span>
                </label>
            </div>
            <div class="wpvivid-two-col wpvivid-ignore" style="<?php echo esc_attr($auto_start_backup_display); ?>">
                <span style="float:right;">
                    <label>
                        <input type="checkbox" option="incremental_backup" name="incremental_files_start_backup" <?php echo esc_attr($incremental_files_start_backup_check); ?> />
                        <span><?php esc_html_e('Perform a full backup immediately when enabling incremental backup', 'wpvivid-backup-pro'); ?></span>
                        <span class="dashicons dashicons-editor-help wpvivid-dashicons-editor-help wpvivid-tooltip">
                            <div class="wpvivid-bottom">
                                <!-- The content you need -->
                                <p><?php esc_html_e('With the option checked, the plugin will perform a full backup of website(files + db) immediately when you enable incremental backups.', 'wpvivid-backup-pro'); ?></p>
        ...
        <div class="wpvivid-one-coloum wpvivid-clear-float" id="wpvivid_edit_incremental_backup" style="padding-bottom:1em;padding-left:0;">
            <input class="button-primary" id="wpvivid_change_incremental_schedule" type="button" value="<?php esc_attr_e('Edit', 'wpvivid-backup-pro'); ?>">
        </div>
        ...
        <div class="wpvivid-one-coloum wpvivid-workflow" id="wpvivid_incremental_backup_deploy" style="display: none;">
            <div class="wpvivid-one-coloum wpvivid-clear-float" style="padding-bottom:1em;padding-left:0;">
                ...
                <span><strong><?php esc_html_e('Incremental Backup Strategy', 'wpvivid-backup-pro'); ?>:</strong></span>
                <div style="padding-left:2em;border-sizing:border-box;">
                    <p><span><strong><?php esc_html_e('Files', 'wpvivid-backup-pro'); ?>: </strong></span><span><?php esc_html_e('Weekly Full Backup + Hourly (or every \'xx\' hours) Incremental Backup', 'wpvivid-backup-pro'); ?></span>
                    <p><span><strong><?php esc_html_e('Database', 'wpvivid-backup-pro'); ?>: </strong></span><span><?php esc_html_e('Database cannot be incrementally backed up, you have to set a backup schedule for database separately.', 'wpvivid-backup-pro'); ?></span>
                </div>
            </div>
            ...
            <table class="widefat" style="margin-bottom:1em;">
                ...
                <tr>
                    ...
                    <th><?php esc_html_e('Backup Cycles', 'wpvivid-backup-pro'); ?></th>
                    <th><?php esc_html_e('Start Time', 'wpvivid-backup-pro'); ?></th>
                </tr>
                </thead>
                <tbody>
                <tr>
                    <td class="row-title"><label for="tablecell"><?php esc_html_e('Files (Full Backup)', 'wpvivid-backup-pro'); ?></label></td>
                    <td>
                        <select id="wpvivid_incrementa_schedule_recurrence" option="incremental_backup" name="recurrence" onchange="change_incremental_backup_recurrence();">
                            <option value="wpvivid_6hours"><?php esc_html_e('Every 6 hours', 'wpvivid-backup-pro'); ?></option>
                            ...
                            <option value="wpvivid_3days"><?php esc_html_e('Every 3 days', 'wpvivid-backup-pro'); ?></option>
                    ...
                    <td>
                        ...
                        <span id="wpvivid_incrementa_schedule_backup_start_day">
                            <?php esc_html_e('Day', 'wpvivid-backup-pro'); ?><select option="incremental_backup" name="recurrence_day">
                                <?php
                                $html='';
                                for($i=1;$i<31;$i++)
                                {
                        ...
                        <span>
                            <?php esc_html_e('Hour', 'wpvivid-backup-pro'); ?><select option="incremental_backup" name="files_current_day_hour" onchange="wpvivid_check_incremental_time('files');">
                                <?php
                                $html='';
                        ...
                        <span>:</span>
                        <span>
                            <?php esc_html_e('Minutes', 'wpvivid-backup-pro'); ?><select option="incremental_backup" name="files_current_day_minute" onchange="wpvivid_check_incremental_time('files');">
                                <?php
                                $html='';
                ...
                <tr>
                    <td class="row-title"><label for="tablecell"><?php esc_html_e('Files (Incremental Backup)', 'wpvivid-backup-pro'); ?></label></td>
                    <td>
                ...
                <tr>
                    <td class="row-title"><label for="tablecell"><?php esc_html_e('Database Backup Cycle', 'wpvivid-backup-pro'); ?></label></td>
                    <td>
                    ...
                    <td>
                        ...
                        <span id="wpvivid_incrementa_schedule_backup_db_start_day">
                            <?php esc_html_e('Day', 'wpvivid-backup-pro'); ?><select option="incremental_backup" name="incremental_db_recurrence_day">
                                <?php
                                $html='';
                                for($i=1;$i<31;$i++)
                                {
                        ...
                        <span>
                            <?php esc_html_e('Hour', 'wpvivid-backup-pro'); ?><select option="incremental_backup" name="db_current_day_hour" onchange="wpvivid_check_incremental_time('db');">
                                <?php
                                $html='';
                                for($hour=0; $hour<24; $hour++)
                                {
                        ...
                        <span>:</span>
                        <span>
                            <?php esc_html_e('Minutes', 'wpvivid-backup-pro'); ?><select option="incremental_backup" name="db_current_day_minute" onchange="wpvivid_check_incremental_time('db');">
                                <?php
                                $html='';
                                for($minute=0; $minute<60; $minute++)
                                {
            ...
            <div class="wpvivid-one-coloum wpvivid-workflow wpvivid-clear-float" style="margin-top:1em;">
                <div style="margin-bottom:1em;">
                    ...
                    <div style="padding-left:2em;">
                        <label class="">
                            <input type="radio" option="incremental_backup" name="save_local_remote" value="local" checked="" /><?php esc_html_e('Save it to localhost', 'wpvivid-backup-pro'); ?>
                        </label>
                        <span style="padding:0 1em;"></span>
                        <label class="">
                            <input type="radio" option="incremental_backup" name="save_local_remote" value="remote" /><?php esc_html_e('Send it to cloud storage', 'wpvivid-backup-pro'); ?>
                        </label>
                ...
                <div>
                    <p><span class="dashicons dashicons-screenoptions wpvivid-dashicons-blue"></span><span><strong><?php esc_html_e('Backup Database Content', 'wpvivid-backup-pro'); ?></strong></span></p>
                ...
                <div>
                    <p><span class="dashicons dashicons-screenoptions wpvivid-dashicons-blue"></span><span><strong><?php esc_html_e('Backup File Content', 'wpvivid-backup-pro'); ?></strong></span></p>
                    <div style="padding:1em;margin-bottom:1em;background:#eaf1fe;border-radius:8px;">
                        <fieldset>
                            <label style="padding-right:2em;">
                ...
                <div>
                    <p>
                        ...
                        <span><strong><?php esc_html_e('Comment the backup', 'wpvivid-backup-pro'); ?></strong><?php esc_html_e('(optional)', 'wpvivid-backup-pro'); ?>: </span><input type="text" option="incremental_backup" name="backup_prefix" id="wpvivid_set_incremental_schedule_prefix" value="<?php echo esc_attr($backup_prefix); ?>" onkeyup="value=value.replace(/[^a-zA-Z0-9._]/g,'')" onpaste="value=value.replace(/[^\a-\z\A-\Z0-9]/g,'')" placeholder="<?php echo esc_attr($backup_prefix); ?>">
                    </p>
                </div>
            </div>
            ...
            <div class="wpvivid-one-coloum wpvivid-clear-float" style="padding-bottom:0;padding-left:0;">
                ...
                <input class="button-primary" type="submit" value="<?php esc_attr_e('Save Changes', 'wpvivid-backup-pro'); ?>" onclick="wpvivid_click_save_incremental_schedule();">
            </div>
        </div>
        <script>
            <?php
            ...
            jQuery('#wpvivid_incremental_backup_switch').click(function(){
                if(jQuery('#wpvivid_incremental_backup_switch').prop('checked')){
                    var enable = 1;
                    var descript = <?php echo wp_json_encode(__('Enabling incremental backup schedule will disable full backup schedules, if any, are you sure to continue?', 'wpvivid-backup-pro')); ?>;
                }
                else{
                    var enable = 0;
                    var descript = <?php echo wp_json_encode(__('Disabling incremental backup will cause the scheduled incremental backup task to not run. Are you sure to continue?', 'wpvivid-backup-pro')); ?>;
                }
                            ...
                            jQuery('[option=incremental_backup_db][name=backup_db]').each(function()
                            {
                                if(jQuery(this).val() === jsonarray.backup_db_type)
                                {
                                    ...
                                    if(jsonarray.backup_db_type === 'custom')
                                    {
                                        ...
                                        var $additional_list = jQuery('#wpvivid_incremental_backup_deploy').find('.wpvivid-additional-database-list');
                                        $additional_list.html('');

                                        if (additional_database_check && typeof jsonarray.additional_database_list !== 'undefined')
                                        {
                                            jQuery.each(jsonarray.additional_database_list, function(database_name, database_info) {
                                                var $item = jQuery('<div></div>')
                                                    .addClass('wpvivid-text-line')
                                                    .attr('database-name', database_name)
                                                    .attr('database-host', database_info.db_host)
                                                    .attr('database-user', database_info.db_user)
                                                    .attr('database-pass', database_info.db_pass);

                                                var $remove_icon = jQuery('<span></span>').addClass(
                                                    'dashicons dashicons-trash ' +
                                                    'wpvivid-icon-16px wpvivid-additional-database-remove'
                                                ).attr('database-name', database_name);

                                                var $database_icon = jQuery('<span></span>').addClass(
                                                    'dashicons dashicons-admin-site-alt3 ' +
                                                    'wpvivid-dashicons-blue wpvivid-icon-16px-nopointer'
                                                );

                                                var $database_text = jQuery('<span></span>')
                                                    .addClass('wpvivid-text-line')
                                                    .attr('option', 'additional_db_custom')
                                                    .attr('name', database_name)
                                                    .text(database_name + '@' + database_info.db_host);
            ...
            function wpvivid_check_incremental_time(type){
                ...
                if(db_current_day === files_current_day){
                    alert(wpvivid_ajax_object_addon.i18n.incremental_schedules_same_start_time)
                }
                var files_current_day=get_wpvivid_sync_time('incremental_backup','files_current_day_hour','files_current_day_minute');
                jQuery('#wpvivid_incremental_files_utc_time').html(files_current_day);
