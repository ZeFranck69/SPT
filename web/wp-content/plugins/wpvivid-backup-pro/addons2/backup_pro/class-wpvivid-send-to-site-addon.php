            ...
            if (isset($_POST['wpvivid_content'])) {
                ...
                if (isset($params['backup_id']))
                {
                    ...
                    if ($backup_list->get_backup_by_id($params['backup_id']) !== false)
                    {
                        ...
                        if (!file_exists($wpvivid_backup_pro->wpvivid_pro_log->GetSaveLogFolder() . $backup_id . '_backup_log.txt')) {
                            $wpvivid_backup_pro->wpvivid_pro_log->CreateLogFile($backup_id . '_backup', 'no_folder', 'transfer');
                            $wpvivid_backup_pro->wpvivid_pro_log->WriteLogHander();
                        } else {
                            $wpvivid_backup_pro->wpvivid_pro_log->OpenLogFile($backup_id . '_backup', 'no_folder');
                        }
