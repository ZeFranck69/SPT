...
class WPvivid_New_Backup_Task
{
    ...
    public function do_backup_job($key)
    {
        ...
        $this->update_sub_task_progress(sprintf(__('Start backing up %s.', 'wpvivid-backup-pro'),$job['backup_type']));
        ...
        $this->update_sub_task_progress(sprintf(__('Backing up %s finished.', 'wpvivid-backup-pro'),$job['backup_type']));
    ...
    public function do_backup_db()
    {
        ...
        $dump_setting=$this->init_db_backup_setting();
        ...
        $dump = new WPvivid_Mysqldump_Addon($this,$dump_setting);
        $dump->connect();
    ...
    public function zip_mysql_dump_files($files,$tables,$find_zero_date=false)
    {
        ...
        if($this->task['options']['encrypt_db']==1)
        {
            $encrypt_db_password = WPvivid_Pro_Option::get_encrypt_db_password();
            if ($encrypt_db_password === false || $encrypt_db_password === '')
            {
                return array(
                    'result' => WPVIVID_PRO_FAILED,
                    'error'  => 'The database encryption password is empty.',
                );
            }
            if (method_exists('WPvivid_Custom_Interface_addon', 'get_vendor_mode')) {
                ...
                if($vendor_mode === 'modern') {
                    $crypt=new WPvivid_Crypt_File_Ex($encrypt_db_password);
                }
                else{
                    $crypt=new WPvivid_Crypt_File($encrypt_db_password);
                }
            }
            else {
                $crypt=new WPvivid_Crypt_File($encrypt_db_password);
    ...
    public function do_backup_additional_db()
    {
        ...
        foreach ($this->task['options']['additional_database_list'] as $database=> $db_info)
        {
            ...
            $dump_setting=$this->init_additional_db_backup_setting($database,$db_info);
            if(!class_exists('WPvivid_Mysqldump_Addon'))
            {
                include WPVIVID_BACKUP_PRO_PLUGIN_DIR . 'addons2/backup_pro/class-wpvivid-mysqldump-addon.php';
            }
    ...
    public function get_backup_task_info()
    {
        ...
        {
            $list_tasks['task_info']['descript']=__('Ready to backup. Progress: 0%, running time: 0second.','wpvivid-backup-pro');
        ...
        else if($list_tasks['status']['str']=='running')
        {
            if($list_tasks['is_canceled'] == false)
            {
                if($list_tasks['data']['type'] == 'upload')
                {
                    ...
                    $list_tasks['task_info']['descript'] = $descript.' '.__('Progress: ', 'wpvivid-backup-pro') . $list_tasks['task_info']['backup_percent'] . ', ' . __('running time: ', 'wpvivid-backup-pro') . $list_tasks['data']['running_time'];
                    ...
                    $list_tasks['task_info']['descript'] = $list_tasks['data']['descript'] . ' '. __('Progress: ', 'wpvivid-backup-pro') . $list_tasks['task_info']['backup_percent'] . ', '. __('running time: ', 'wpvivid-backup-pro') . $list_tasks['data']['running_time'];
                }
                $list_tasks['task_info']['css_btn_cancel']='pointer-events: auto; opacity: 1;';
                $list_tasks['task_info']['css_btn_log']='pointer-events: auto; opacity: 1;';
            }
            else{
                $list_tasks['task_info']['descript']=__('The backup will be canceled after backing up the current chunk ends.','wpvivid-backup-pro');
                $list_tasks['task_info']['css_btn_cancel']='pointer-events: none; opacity: 0.4;';
                $list_tasks['task_info']['css_btn_log']='pointer-events: auto; opacity: 1;';
            }
        }
    ...
    public function get_backup_tasks_progress()
    {
        ...
        {
            $ret['descript']=__($this->task['data'][$ret['type']]['sub_job'][$ret['doing']]['progress'], 'wpvivid-backup-pro');
    ...
    public function get_download_progress($backup_id, $files){
                ...
                if(empty($this->config['remote'])) {
                    ...
                    if ($task === false) {
                        ...
                        $ret['files'][$file['file_name']]['html']='<div style="float:left;margin:10px 10px 10px 0;text-align:center; width:180px;">
                                                                  ...
                                                                  <span id=\''.$backup_id.'-text-part-'.$file_part_num.'\'><a onclick="wpvivid_prepare_download(\''.$file_part_num.'\', \''.$backup_id.'\', \''.$file['file_name'].'\');" style="cursor: pointer;">'.esc_html__('Prepare to Download', 'wpvivid-backup-pro').'</a></span></br>
                        ...
                        elseif($task['status'] === 'completed'){
                            ...
                            $ret['files'][$file['file_name']]['html']='<div style="float:left;margin:10px 10px 10px 0;text-align:center; width:180px;">
                                                                 <span>Part'.$format_part.'</span></br>
                                                                 <span id=\''.$backup_id.'-text-part-'.$file_part_num.'\'><a onclick="wpvivid_download(\''.$backup_id.'\', \''.$check_type.'\', \''.$file['file_name'].'\');" style="cursor: pointer;">'.esc_html__('Download', 'wpvivid-backup-pro').'</a></span></br>
        ...
        if ($file_count % 2 != 0) {
            ...
            $ret['place_html']='<div style="float:left;margin:10px 10px 10px 0;text-align:center; width:180px; color:#cccccc;">
                                   <span>Part'.$format_part.'</span></br>
                                   <span>'.esc_html__('Download', 'wpvivid-backup-pro').'</span></br>
                                   ...
                                   <span>'.esc_html__('size:', 'wpvivid-backup-pro').'</span><span>0</span>
                                   </div>';
        }
        else{
            $ret['place_html']='';
