...
class WPvivid_Log_List extends WP_List_Table
{
    ...
    public function get_columns()
    {
        ...
        $columns['wpvivid_log_type'] = __( 'Log Type', 'wpvivid-backup-pro' );
        $columns['wpvivid_log_file_name'] =__( 'Log File Name', 'wpvivid-backup-pro'  );
        $columns['wpvivid_log_action'] = __( 'Action', 'wpvivid-backup-pro'  );
        $columns['wpvivid_download'] = __( 'Download', 'wpvivid-backup-pro'  );
    ...
    {
        $localtime = WPvivid_Time::parse_local_datetime_to_timestamp($log['time']);
        if ($localtime === false) {
            echo '<td><label for="tablecell">'.esc_html(trim($log['time'])).'</label></td>';
            return;
        }
    ...
    public function column_wpvivid_log_action( $log )
    {
        $html='<span class="dashicons dashicons-welcome-write-blog wpvivid-dashicons-blue open-log" style="cursor:pointer;" log="'.$log['file_name'].'"></span>
			    <span style="cursor:pointer; margin:0;" class="open-log" log="'.$log['file_name'].'">'.esc_html__('Log', 'wpvivid-backup-pro').'</span>';
        echo $html;
    }
    ...
    public function column_wpvivid_download( $log )
    {
        $html='<div class="download-log" log="'.$log['file_name'].'" style="cursor:pointer;" title="Prepare to download the backup">
                    <span class="dashicons dashicons-arrow-down-alt wpvivid-dashicons-blue"></span><span>'.esc_html__('Download', 'wpvivid-backup-pro').'</span>
               </div>';
        echo $html;
    }
    ...
    public function download_log()
    {
        ...
        try
        {
            if (isset($_REQUEST['log']))
            {
                ...
                {
                    $message= __('Log does not exist. It might have been deleted or lost during a website migration.', 'wpvivid-backup-pro');
                    $retry_url = $admin_url . 'admin.php?page=' . apply_filters('wpvivid_white_label_plugin_name', 'wpvivid-debug');
                    echo wp_kses_post(sprintf(__('%1$s <a href="%2$s">retry</a> again.', 'wpvivid-backup-pro'), esc_html($message), esc_url($retry_url)));
                    die();
                }
                ...
                $message = __('Reading the log failed. Please try again.', 'wpvivid-backup-pro');
                $retry_url = $admin_url . 'admin.php?page=' . apply_filters('wpvivid_white_label_plugin_name', 'wpvivid-debug');
                    echo wp_kses_post(sprintf(__('%1$s <a href="%2$s">retry</a> again.', 'wpvivid-backup-pro'), esc_html($message), esc_url($retry_url)));
                die();
            }
