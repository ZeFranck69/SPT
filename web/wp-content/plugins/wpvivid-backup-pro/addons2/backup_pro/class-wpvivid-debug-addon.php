...
class WPvivid_Debug_addon
{
    ...
    public function get_dashboard_menu($submenus,$parent_slug)
    {
        ...
        if($display)
        {
            ...
            $submenu['page_title'] = apply_filters('wpvivid_white_label_display', __('Debug', 'wpvivid-backup-pro'));
            $submenu['menu_title'] = __('Debug', 'wpvivid-backup-pro');
    ...
    public function get_toolbar_menus($toolbar_menus)
    {
        ...
        if($display) {
            ...
            $menu['title'] = __('Debug', 'wpvivid-backup-pro');
    ...
    public function wpvivid_send_debug_info()
    {
        ...
        try {
            ...
            {
                $ret['result'] = 'failed';
                $ret['error'] = __('User\'s email address is required.', 'wpvivid-backup-pro');
            } else {
                $pattern = '/^[a-z0-9]+([._-][a-z0-9]+)*@([0-9a-z-]+\.[a-z]{2,14}(\.[a-z]{2})?)$/i';
                if (!preg_match($pattern, $_POST['user_mail'])) {
                    $ret['result'] = 'failed';
                    $ret['error'] = __('Please enter a valid email address.', 'wpvivid-backup-pro');
                } else {
                    $wpvivid_backup_pro->ajax_check_security('wpvivid-can-backup');
                            ...
                            <div class="wpvivid-backup">
                                <div class="wpvivid-welcome-bar wpvivid-clear-float">
                                    <div class="wpvivid-welcome-bar-left">
                                        <p><span class="dashicons  dashicons-buddicons-replies wpvivid-dashicons-large wpvivid-dashicons-orange"></span><span class="wpvivid-page-title"><?php esc_html_e('Debug', 'wpvivid-backup-pro'); ?></span></p>
                                        <span class="about-description"><?php esc_html_e('This page provides detailed system information to assist in efficient troubleshooting.', 'wpvivid-backup-pro'); ?></span>
                                    </div>
                                    ...
                                    <div class="wpvivid-one-coloum">
                                        <div class="wpvivid-one-coloum wpvivid-workflow wpvivid-clear-float">
                                            <h2 style="padding-left:0;"><span class="dashicons dashicons-email-alt wpvivid-dashicons-green"></span>
                                                <span><?php esc_html_e('Debug Information', 'wpvivid-backup-pro'); ?></span>
                                            </h2>
                                            <table class="widefat">
                                                <thead class="website-info-head">
                                                <tr>
                                                    <th class="row-title" style="min-width: 260px;"><?php esc_html_e('Website Info Key', 'wpvivid-backup-pro'); ?></th>
                                                    <th><?php esc_html_e('Website Info Value', 'wpvivid-backup-pro'); ?></th>
                                                </tr>
                                                </thead>
                                            ...
                                            <p></p>
                                            <input class="button-primary" type="submit" id="wpvivid_download_website_info" name="download-website-info" value="<?php esc_attr_e('Download', 'wpvivid-backup-pro'); ?>">
                                        </div>
                                    </div>
                                </div>
                ...
                wpvivid_post_request_addon(ajax_data, function (data) {
                    try {
                        ...
                        if (jsonarray.result === "success") {
                            alert(wpvivid_ajax_object_addon.i18n.send_succeeded);
                        }
                        else {
