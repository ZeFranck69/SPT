...
class WPvivid_FTPClass_addon extends WPvivid_Remote_addon
{
    ...
    public function __construct($options=array())
    {
        if(empty($options))
        {
            if (!defined('WPVIVID_INIT_STORAGE_TAB_FTP')) {
                ...
                add_filter('wpvivid_edit_remote_options_ex', array($this, 'edit_remote_options'), 10, 3);
    ...
    {
        if (!isset($old_remote['type']) || $old_remote['type'] !== WPVIVID_REMOTE_FTP) {
            return $old_remote;
        }

        $new_fields = array(
            'use_wp_config_crypt',
            'remote_crypt_test_passed',
            'remote_crypt_test_key_hash',
            'remote_crypt_test_password_hash',
            'generated_remote_crypt_key_hash'
        );

        foreach ($new_fields as $field)
        {
            if (isset($remote_options[$field]))
            {
                $old_remote[$field] = $remote_options[$field];
            }
        }
    ...
    private function wpvivid_output_remote_crypt_js()
    {
        ?>
        <script>
            if (typeof window.wpvivid_remote_crypt_js_loaded === 'undefined')
            {
                window.wpvivid_remote_crypt_js_loaded = true;

                function wpvivid_get_current_remote_crypt_container(obj)
                {
                    var container = obj.closest('#storage_account_ftp, #remote_storage_edit_ftp');

                    if (container.length === 0) {
                        container = obj.closest('.storage-account-page');
                    }

                    return container;
                }

                function wpvivid_get_remote_password_from_container(container)
                {
                    var password_obj = container.find('input[name="password"]');
                ...
                function wpvivid_reset_remote_crypt_test_status(container)
                {
                    container.find('input[name="remote_crypt_test_passed"]').val('0');
                    container.find('input[name="remote_crypt_test_key_hash"]').val('');
                    container.find('input[name="remote_crypt_test_password_hash"]').val('');
                }

                function wpvivid_show_remote_crypt_key(container, jsonarray)
                {
                    var output = container.find('.wpvivid-remote-crypt-key-output');
                    var message = '';

                    if (parseInt(jsonarray.has_current_define) === 1) {
                        message += 'Warning: An encryption key is already defined in wp-config.php. ';
                        message += 'Replacing it may cause existing FTP storage connection to stop working.' + "\n\n";
                    }
                    else {
                        message += 'Please add the following line to wp-config.php before adding this remote storage:' + "\n\n";
                    }

                    message += jsonarray.define;

                    if (output.length > 0) {
                        output.val(message);
                        output.closest('.wpvivid-remote-crypt-key-output-row').show();
                        output.show();
                    }
                    else {
                        alert(message);
                ...
                jQuery(document).on('click', '.wpvivid-generate-ftp-crypt-key', function()
                {
                    var obj = jQuery(this);
                    var container = wpvivid_get_current_remote_crypt_container(obj);

                    wpvivid_reset_remote_crypt_test_status(container);
                    container.find('input[name="generated_remote_crypt_key_hash"]').val('');

                    var ajax_data = {
                        'action': 'wpvivid_generate_remote_crypt_key'
                    };
                    ...
                    wpvivid_post_request_addon(ajax_data, function(data)
                    {
                        try {
                            ...
                            if (jsonarray.result === 'success') {
                                container.find('input[name="generated_remote_crypt_key_hash"]').val(jsonarray.key_hash);
                                wpvivid_show_remote_crypt_key(container, jsonarray);
                            }
                            else {
                ...
                jQuery(document).on('click', '.wpvivid-test-ftp-crypt-key', function()
                {
                    var obj = jQuery(this);
                    var container = wpvivid_get_current_remote_crypt_container(obj);
                    var password = wpvivid_get_remote_password_from_container(container);

                    if (password === '') {
                        alert(wpvivid_ajax_object_addon.i18n.the_ftp_password_is_required);
                        return;
                    }

                    if (password === '********') {
                        alert(wpvivid_ajax_object_addon.i18n.please_enter_the_ftp_password_before_testing_encryption);
                        return;
                    }

                    var ajax_data = {
                        'action': 'wpvivid_test_remote_crypt_key',
                        'password': password,
                        'generated_key_hash': container.find('input[name="generated_remote_crypt_key_hash"]').val()
                    };
                    ...
                    wpvivid_post_request_addon(ajax_data, function(data)
                    {
                        try {
                            ...
                            if (jsonarray.result === 'success') {
                                container.find('input[name="remote_crypt_test_passed"]').val('1');
                                container.find('input[name="remote_crypt_test_key_hash"]').val(jsonarray.key_hash);
                                container.find('input[name="remote_crypt_test_password_hash"]').val(jsonarray.password_hash);
                                alert(jsonarray.notice);
                            }
                            else {
                                wpvivid_reset_remote_crypt_test_status(container);
                                alert(jsonarray.error);
                ...
                jQuery(document).on('input change', '#storage_account_ftp input[name="password"], #remote_storage_edit_ftp input[name="password"]', function()
                {
                    var container = wpvivid_get_current_remote_crypt_container(jQuery(this));
                    wpvivid_reset_remote_crypt_test_status(container);
                });

                jQuery(document).on('change', '#storage_account_ftp input[name="use_wp_config_crypt"], #remote_storage_edit_ftp input[name="use_wp_config_crypt"]', function()
                {
                    var container = wpvivid_get_current_remote_crypt_container(jQuery(this));
                    wpvivid_reset_remote_crypt_test_status(container);
                });
    ...
    public function wpvivid_add_storage_page_ftp()
    {
        ...
        <div id="storage_account_ftp" class="storage-account-page">
            <div style="padding: 0 10px 10px 0;">
                <strong><?php esc_html_e('Enter Your FTP Account', 'wpvivid-backup-pro'); ?></strong>
            </div>
            <table class="wp-list-table widefat plugins" style="width:100%;">
                ...
                <form>
                    <tr>
                        ...
                        <td class="column-description desc">
                            <div class="wpvivid-storage-form-desc">
                                <i><?php esc_html_e('A name to help you identify the storage if you have multiple remote storage connected.', 'wpvivid-backup-pro'); ?></i>
                            </div>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td class="column-description desc">
                            <div class="wpvivid-storage-form-desc">
                                <i><?php esc_html_e('Enter the server address.', 'wpvivid-backup-pro'); ?></i>
                            </div>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td class="column-description desc">
                            <div class="wpvivid-storage-form-desc">
                                <i><?php esc_html_e('Enter the custom server port.', 'wpvivid-backup-pro'); ?></i>
                            </div>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td class="column-description desc">
                            <div class="wpvivid-storage-form-desc">
                                <i><?php esc_html_e('Enter your FTP server user name.', 'wpvivid-backup-pro'); ?></i>
                            </div>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td class="column-description desc">
                            <div class="wpvivid-storage-form-desc">
                                <i><?php esc_html_e('Enter the FTP server password.', 'wpvivid-backup-pro'); ?></i>
                            </div>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td class="plugin-title column-primary">
                            <div class="wpvivid-storage-select">
                                <label>
                                    <input type="checkbox" option="ftp" name="use_wp_config_crypt" />
                                    <?php esc_html_e('Encrypt the FTP password using a key defined in wp-config.php.', 'wpvivid-backup-pro'); ?>
                                </label>
                                <div style="margin-top:8px;">
                                    <input type="button" class="button wpvivid-generate-ftp-crypt-key" value="<?php esc_attr_e('Generate Key', 'wpvivid-backup-pro'); ?>" />
                                    <input type="button" class="button wpvivid-test-ftp-crypt-key" value="<?php esc_attr_e('Test Encryption', 'wpvivid-backup-pro'); ?>" />
                                </div>

                                <input type="hidden" option="ftp" name="remote_crypt_test_passed" value="0" />
                                <input type="hidden" option="ftp" name="remote_crypt_test_key_hash" value="" />
                                <input type="hidden" option="ftp" name="remote_crypt_test_password_hash" value="" />
                                <input type="hidden" option="ftp" name="generated_remote_crypt_key_hash" value="" />
                            </div>
                        </td>
                        <td class="column-description desc">
                            <div class="wpvivid-storage-form-desc">
                                <i><?php esc_html_e('Generate an encryption key, add it to wp-config.php, and test the encryption before adding the FTP storage.', 'wpvivid-backup-pro'); ?></i>
                            </div>
                        </td>
                    </tr>

                    <tr class="wpvivid-remote-crypt-key-output-row" style="display:none;">
                        <td colspan="2" style="padding-top:0;">
                            <textarea class="large-text wpvivid-remote-crypt-key-output" readonly style="width:100%;max-width:100%;box-sizing:border-box;height:90px;background:#fff;color:#1d2327;"></textarea>
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td class="column-description desc">
                            <div class="wpvivid-storage-form-desc">
                                <i><?php echo sprintf(__('Enter an existing absolute path in which you want to create a parent folder for holding %s folders.', 'wpvivid-backup-pro'), apply_filters('wpvivid_white_label_display', 'WPvivid backup')); ?></i>
                            </div>
                        </td>
                    </tr>
                    <tr>
                        <td class="plugin-title column-primary">
                            <div class="wpvivid-storage-form">
                                <input type="text" class="regular-text" autocomplete="off" option="ftp" name="root_path" value="<?php echo esc_attr(apply_filters('wpvivid_white_label_remote_root_path', 'wpvividbackuppro')); ?>" />
                            </div>
                        </td>
                        <td class="column-description desc">
                            <div class="wpvivid-storage-form-desc">
                                <i><?php echo sprintf(__('Customize a parent folder under the absolute path for holding %s folders.', 'wpvivid-backup-pro'), apply_filters('wpvivid_white_label_display', 'WPvivid backup')); ?></i>
                            </div>
                        </td>
                    </tr>
                    <tr>
                        <td class="plugin-title column-primary">
                            <div class="wpvivid-storage-form">
                                <input type="text" class="regular-text" autocomplete="off" option="ftp" name="custom_path" placeholder="Custom Path(e.g. myfolder)" value="<?php echo esc_attr($wpvivid_backup_pro->func->swtich_domain_to_folder_name(home_url())); ?>" />
                            </div>
                        </td>
                        <td class="column-description desc">
                            <div class="wpvivid-storage-form-desc">
                                <i><?php echo sprintf(__('Customize the name of folder under the parent folder where you want to store %s backups.', 'wpvivid-backup-pro'), apply_filters('wpvivid_white_label_display', WPVIVID_PRO_PLUGIN_SLUG)); ?></i>
                            </div>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td class="plugin-title column-primary">
                            <div class="wpvivid-storage-select">
                                <label>
                                    <input type="checkbox" option="ftp" name="default" checked /> <?php esc_html_e('Set as the default remote storage.', 'wpvivid-backup-pro'); ?>
                                </label>
                            </div>
                        </td>
                        <td class="column-description desc">
                            <div class="wpvivid-storage-form-desc">
                                <i><?php esc_html_e('Once checked, backups sent to a remote storage destination will be uploaded to this storage by default.', 'wpvivid-backup-pro'); ?></i>
                            </div>
                        </td>
                    </tr>
                    <tr>
                        <td class="plugin-title column-primary">
                            <div class="wpvivid-storage-select">
                                <label>
                                    <input type="checkbox" option="ftp" name="use_ftps" /><?php esc_html_e('Check this option to enable FTP-SSL connection.', 'wpvivid-backup-pro'); ?>
                                </label>
                            </div>
                        </td>
                        <td class="column-description desc">
                            <div class="wpvivid-storage-form-desc">
                                <i><?php esc_html_e('Check this option to enable FTP-SSL connection while transferring files. Make sure the FTP server you are configuring supports FTPS connections.', 'wpvivid-backup-pro'); ?></i>
                            </div>
                        </td>
                    </tr>
                    <tr>
                        <td class="plugin-title column-primary">
                            <div class="wpvivid-storage-select">
                                <label>
                                    <input type="checkbox" option="ftp" name="passive" checked /><?php esc_html_e('Uncheck this to enable FTP active mode.', 'wpvivid-backup-pro'); ?>
                                </label>
                            </div>
                        </td>
                        <td class="column-description desc">
                            <div class="wpvivid-storage-form-desc">
                                <i><?php esc_html_e('Uncheck the option to use FTP active mode when transferring files. Make sure the FTP server you are configuring supports the active FTP mode.', 'wpvivid-backup-pro'); ?></i>
                            </div>
                        </td>
                    </tr>
                </form>
                <tr>
                    <td class="plugin-title column-primary">
                        <div class="wpvivid-storage-form">
                            <input class="button-primary" type="submit" option="add-remote" value="<?php esc_attr_e('Test and Add', 'wpvivid-backup-pro'); ?>">
                        </div>
                    </td>
                    <td class="column-description desc">
                        <div class="wpvivid-storage-form-desc">
                            <i><?php esc_html_e('Click the button to connect to FTP server and add it to the storage list below.', 'wpvivid-backup-pro'); ?></i>
                        </div>
                    </td>
                </tr>
                </tbody>
        ...
        <?php
        $this->wpvivid_output_remote_crypt_js();
    }
        ...
        <div id="remote_storage_edit_ftp">
            ...
            <table class="wp-list-table widefat plugins" style="width:100%;">
                ...
                <form>
                    ...
                    <tr>
                        <td class="plugin-title column-primary">
                            <div class="wpvivid-storage-select">
                                <label>
                                    <input type="checkbox" option="edit-ftp" name="use_wp_config_crypt" />
                                    <?php esc_html_e('Encrypt the FTP password using a key defined in wp-config.php.', 'wpvivid-backup-pro'); ?>
                                </label>
                                <div style="margin-top:8px;">
                                    <input type="button" class="button wpvivid-generate-ftp-crypt-key" value="<?php esc_attr_e('Generate Key', 'wpvivid-backup-pro'); ?>" />
                                ...
                                <input type="hidden" option="edit-ftp" name="remote_crypt_test_passed" value="0" />
                                <input type="hidden" option="edit-ftp" name="remote_crypt_test_key_hash" value="" />
                                <input type="hidden" option="edit-ftp" name="remote_crypt_test_password_hash" value="" />
                                <input type="hidden" option="edit-ftp" name="generated_remote_crypt_key_hash" value="" />
                            </div>
                        </td>
                        <td class="column-description desc">
                            <div class="wpvivid-storage-form-desc">
                    ...
                    <tr>
                        <td class="plugin-title column-primary">
                            <div class="wpvivid-storage-form">
                                <input type="text" class="regular-text" autocomplete="off" option="edit-ftp" name="root_path" value="<?php echo esc_attr(apply_filters('wpvivid_white_label_remote_root_path', 'wpvividbackuppro')); ?>" />
                            </div>
                        </td>
                        <td class="column-description desc">
                            <div class="wpvivid-storage-form-desc">
                    ...
                    <tr>
                        <td class="plugin-title column-primary">
                            <div class="wpvivid-storage-select">
                                <label>
                                    <input type="checkbox" option="edit-ftp" name="use_ftps" /><?php esc_html_e('Check this option to enable FTP-SSL connection.', 'wpvivid-backup-pro'); ?>
                                </label>
                            </div>
                        </td>
                        <td class="column-description desc">
                    ...
                    <tr>
                        <td class="plugin-title column-primary">
                            <div class="wpvivid-storage-select">
                                <label>
                                    <input type="checkbox" option="edit-ftp" name="passive" checked /><?php esc_html_e('Uncheck this to enable FTP active mode.', 'wpvivid-backup-pro'); ?>
                                </label>
                            </div>
                        </td>
                        <td class="column-description desc">
                ...
                <tr>
                    <td class="plugin-title column-primary">
                        <div class="wpvivid-storage-form">
                            <input class="button-primary" type="submit" option="edit-remote" value="<?php esc_attr_e('Save Changes', 'wpvivid-backup-pro'); ?>">
                        </div>
                    </td>
                    <td class="column-description desc">
                        <div class="wpvivid-storage-form-desc">
                            <i><?php esc_html_e('Click the button to save the changes.', 'wpvivid-backup-pro'); ?></i>
                        </div>
                    </td>
                </tr>
                </tbody>
    ...
    private function wpvivid_get_ftp_password()
    {
        if (!isset($this->options['password'])) {
            return '';
        }

        if (isset($this->options['is_encrypt'])) {
            if (intval($this->options['is_encrypt']) === 2) {
                if (!class_exists('WPvivid_Remote_Password_Crypt_Ex')) {
                ...
                $crypt = new WPvivid_Remote_Password_Crypt_Ex();

                if (!$crypt->load_key_from_config()) {
                    return false;
                }

                $password = $crypt->decrypt($this->options['password']);
            ...
            if (intval($this->options['is_encrypt']) === 1) {
                return base64_decode($this->options['password']);
            }
        }
    ...
    private function wpvivid_get_valid_ftp_password(&$ret)
    {
        $password = $this->wpvivid_get_ftp_password();

        if ($password === false) {
            $ret['result'] = 'failed';
            $ret['error'] = 'Failed to decrypt the remote storage password. Please ensure a correct encryption key is defined in wp-config.php.';
            return false;
        }

        if ($password === '') {
            $ret['result'] = 'failed';
            $ret['error'] = 'The FTP password is required.';
            return false;
        }
    ...
    public function test_connect()
    {
        ...
        $ret = array();
        $password = $this->wpvivid_get_valid_ftp_password($ret);
        if ($password === false) {
            return $ret;
        }
        ...
        if($ret['result']=='success')
        {
            ...
            if($ret['result']=='success')
            {
                ...
                if($ret['result']=='success')
                {
                    ...
                    {
                        return array('result'=>WPVIVID_PRO_FAILED,'error'=>__('Failed to add FTP storage. It can be because the FTP folder permissions are insufficient, or calling PHP ftp_put function of your web server failed. Please make sure the folder has write permission and the ftp_put function works properly.','wpvivid-backup-pro'));
                    }
                    @unlink($temp_path);
    ...
    public function sanitize_options($skip_name='')
    {
        ...
        {
            $ret['error']=__('Warning: An alias for remote storage is required.','wpvivid-backup-pro');
            return $ret;
        }
        ...
        foreach ($remoteslist as $key=>$value)
        {
            ...
            {
                $ret['error']=__('Warning: The alias already exists in storage list.','wpvivid-backup-pro');
                return $ret;
            }
        }
        ...
        {
            $ret['error']=__('Warning: The FTP server is required.','wpvivid-backup-pro');
            return $ret;
        }
        ...
        if(!isset($this->options['port'])){
            $ret['error']=__('Warning: The servers port is required.','wpvivid-backup-pro');
            return $ret;
        }
        ...
        {
            $ret['error']=__('Warning: The FTP login is required.','wpvivid-backup-pro');
            return $ret;
        }
        ...
        {
            $ret['error']=__('Warning: The FTP password is required.','wpvivid-backup-pro');
            return $ret;
        }
        ...
        if (isset($this->options['use_wp_config_crypt']) && intval($this->options['use_wp_config_crypt']) === 1) {
            if (!isset($this->options['remote_crypt_test_passed']) || intval($this->options['remote_crypt_test_passed']) !== 1) {
                $ret['error'] = __('Please test the encryption successfully before adding this remote storage.','wpvivid-backup-pro');
                return $ret;
            }
            ...
            if (!class_exists('WPvivid_Remote_Password_Crypt_Ex')) {
                $ret['error'] = __('Failed to load the encryption class. Please ensure WPvivid Backup Pro is up to date.','wpvivid-backup-pro');
                return $ret;
            }
            ...
            if (!$crypt->load_key_from_config()) {
                $ret['error'] = __('The encryption key is missing or invalid in wp-config.php. Please add a valid key.','wpvivid-backup-pro');
                return $ret;
            }

            $encrypted_password = $crypt->encrypt($this->options['password']);

            if ($encrypted_password === false) {
                $ret['error'] = __('Failed to encrypt the remote storage password. Please test the encryption first.','wpvivid-backup-pro');
                return $ret;
            }

            $decrypted_password = $crypt->decrypt($encrypted_password);
            if ($decrypted_password !== $this->options['password']) {
                $ret['error'] = __('Failed to verify the encrypted remote storage password. Please test the encryption first.','wpvivid-backup-pro');
                return $ret;
            }

            $this->options['password'] = $encrypted_password;
            $this->options['is_encrypt'] = 2;
            $this->options['encrypt_method'] = 'remote_password_key';
        }
        else {
            $this->options['password'] = base64_encode($this->options['password']);
            $this->options['is_encrypt'] = 1;

            if (isset($this->options['encrypt_method'])) {
                unset($this->options['encrypt_method']);
            }
        }

        unset($this->options['use_wp_config_crypt']);
        unset($this->options['remote_crypt_test_passed']);
        unset($this->options['remote_crypt_test_key_hash']);
        unset($this->options['remote_crypt_test_password_hash']);
        unset($this->options['generated_remote_crypt_key_hash']);

        if(!isset($this->options['path'])||empty($this->options['path']))
        {
            $ret['error']=__('Warning: The storage path is required.','wpvivid-backup-pro');
            return $ret;
        }
        ...
        {
            $ret['error']=__('Warning: Root directory is forbidden to set to \'/\'.','wpvivid-backup-pro');
            return $ret;
        }

        if(!isset($this->options['root_path'])||empty($this->options['root_path']))
        {
            $ret['error']=__('Warning: The root path is required.','wpvivid-backup-pro');
            return $ret;
        }
        ...
        {
            $ret['error']=__('The backup folder name cannot be \'/\'','wpvivid-backup-pro');
            return $ret;
        }

        if(!isset($this->options['custom_path'])||empty($this->options['custom_path']))
        {
            $ret['error']=__('Warning: The custom path is required.','wpvivid-backup-pro');
            return $ret;
        }
        ...
        if(isset($this->options['use_remote_retention']) && $this->options['use_remote_retention'] == '1')
        {
            if (!isset($this->options['backup_retain'])) {
                $ret['error'] = __('Warning: You have not set the backup retention policy for this storage. Please set the policy or uncheck the option.','wpvivid-backup-pro');
                return $ret;
            }
    ...
    public function do_connect($server,$username,$password,$port = 21)
    {
        ...
        if($conn)
        {
            ...
            {
                return array('result'=>WPVIVID_PRO_FAILED,'error'=>__('Login failed. You have entered the incorrect credential(s). Please try again.','wpvivid-backup-pro'));
            }
        }
        else{
            return array('result'=>WPVIVID_PRO_FAILED,'error'=>__('Login failed. The connection has timed out. Please try again later.','wpvivid-backup-pro'));
        }
    }
                ...
                if($part !== '') {
                    if (!@ftp_chdir($conn, $part)) {
                        if (!ftp_mkdir($conn, $part)) {
                            return array('result' => WPVIVID_PRO_FAILED, 'error' => __('Failed to create a backup. Make sure you have sufficient privileges to perform the operation.','wpvivid-backup-pro'));
                        }
