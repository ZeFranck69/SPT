...
class WPvivid_Custom_Interface_addon
{
    ...
    public function wpvivid_get_others(){
        ...
        $ret = '<div class="postbox schedule-tab-block setting-page-content">
                    <div> 
                        ...
                        <input type="button" id="wpvivid_select_tree_node" value="'.esc_attr__('Select This Node', 'wpvivid-backup-pro').'" />
                    </div>
                </div>';
        return $ret;
    }
    ...
    public function wpvivid_get_default_exclude_list_html()
    {
        $ret='';

        $exclude_default = apply_filters('wpvivid_default_exclude_folders', array());

        foreach($exclude_default as $value)
        {
            if($value['type']=='folder')
            {
                $class_type='dashicons dashicons-category wpvivid-dashicons-orange wpvivid-icon-16px-nopointer';
            ...
            foreach ($exclude_path['content_option']['exclude_content_list'] as $index => $value) {
                $ret .= '<ul>
                            <li>
                                ...
                                <div class="wpvivid-custom-li-close" onclick="wpvivid_remove_tree(this);" title="'.esc_attr__('Remove', 'wpvivid-backup-pro').'" style="cursor: pointer;">X</div>
                            </li>
                         </ul>';
            }
        }
    ...
    public static function delete_backup_by_id($backup_id,$force=0)
    {
        ...
        if($backup_item->is_lock())
        {
            ...
            {
                $ret['result']='failed';
                $ret['error']=__('Unable to delete the locked backup. Please unlock it first and try again.', 'wpvivid-backup-pro');
                return $ret;
            }
        }
