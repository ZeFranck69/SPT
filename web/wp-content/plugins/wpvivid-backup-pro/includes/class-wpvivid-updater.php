...
class WPvivid_Updater
{
    ...
    private function get_key($url){
        ...
        $response=wp_remote_get($url.'?request=get_key',$options);
        if(!is_wp_error($response) && ($response['response']['code'] == 200))
        {
            ...
            {
                $public_key=base64_decode($json['public_key']);
                if($public_key==null)
                {
                    return false;
                }
    ...
    private function get_cached_plugin_info() {
        $cache = get_transient( 'wpvivid_pro_plugin_info_cache' );
    ...
    private function request_remote_plugin_info() {
        $url  = 'https://download.wpvivid.com/download/wpvivid_pro_info_data';
        $public_key  = get_option( 'wpvivid_connect_key', '' );
        if ( empty( $public_key ) ) {
            $public_key = $this->get_key( $url );
            if ( $public_key === false ) {
                return false;
            }
        }
        ...
        $request_url = $url . '?request=get_plugin_info&data=' . rawurlencode( base64_encode( $data ) );
    ...
    private function get_wpvivid_pro_plugin_info_data($force = false) {
        if ( ! $force ) {
            $cached = $this->get_cached_plugin_info();
            if ( $cached !== false ) {
                return $cached;
            }
        }

        $data = $this->request_remote_plugin_info();

        if ( $data === false ) {
            return false;
        }

        set_transient( 'wpvivid_pro_plugin_info_cache', $data, DAY_IN_SECONDS );
            ...
            if($dashboard_info!==false)
            {
                ...
                if(time()>$show_time)
                {
                    if(is_admin() && current_user_can('administrator'))
                    {
                        ...
                        $message .= sprintf(__('There is a new version of %s Pro available.', 'wpvivid-backup-pro'), apply_filters('wpvivid_white_label_display', 'WPvivid Plugins')).' <a href="'. $url. '">Update now</a> to Version ' . $version . ' </div>';
                ...
                if(time()>$show_time)
                {
                    if(is_admin() && current_user_can('administrator'))
                    {
                        ...
                        $message = sprintf(
                                ...
                                __( 'There is a new version of %1$s Pro available. <a href="%2$s">Update now</a> to Version %3$s', 'wpvivid-backup-pro' ),
    ...
    public function pro_update_row($file, $plugin_data)
    {
        ...
        if ( ! empty( $update_cache->response[ $this->plugin_basename ] ) && version_compare( WPVIVID_BACKUP_PRO_VERSION, $version_info->new_version, '<' ) )
        {
            ...
            $admin_url = apply_filters('wpvivid_get_admin_url', '').apply_filters('wpvivid_white_label_page_redirect', 'admin.php?page=wpvivid-license', 'wpvivid-license');
            ...
            if ( empty( $version_info->download_link ) )
            {
                ...
                printf(
                    __( 'There is a new version of %1$s available.<a href="%2$s" class="update-link" %3$s> Update now%4$s.', 'wpvivid-backup-pro' ),
                    $plugin_data['Name'],
