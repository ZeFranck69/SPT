...
class WPvivid_Interface_MainWP_addon
{
    ...
    public function wpvivid_list_tasks_addon_mainwp_ex()
    {
        ...
        foreach ($tasks as $task)
        {
            ...
            $list_tasks[$task['id']]['task_info']['mainwp_support_new_backup_progress'] = true;
            ...
            $last_log_data = $this->wpvivid_mainwp_render_log_last_lines($log_path);
            $log_data_html = '';
            if(!empty($last_log_data))
            {
                foreach ($last_log_data as $line)
                {
                    $log_data_html .= '<p>' . esc_html($line) . '</p>';
                }
            }
            $list_tasks[$task['id']]['task_info']['mainwp_log_html'] = $log_data_html;
    ...
    public function wpvivid_view_log_addon_mainwp($data){
        ...
        {
            $ret['result'] = 'failed';
            $ret['error'] = __('Log does not exist. It might have been deleted or lost during a website migration.', 'wpvivid-backup-pro');
            return $ret;
        }
        ...
        if (!$file) {
            $ret['result'] = 'failed';
            $ret['error'] = __('Unable to open the log file.', 'wpvivid-backup-pro');
            return $ret;
        }
    ...
    public function wpvivid_archieve_report_addon_mainwp($data){
        $message = WPvivid_Pro_Option::get_last_backup_message(array());
        $ret = array();
        if(!empty($message['id'])) {
            $ret['id'] = $message['id'];
            $ret['status'] = $message['status'];
            $ret['status']['start_time'] = WPvivid_Time::format_local("M d, Y H:i", $ret['status']['start_time']);
            $ret['status']['run_time'] = WPvivid_Time::format_local("M d, Y H:i", $ret['status']['run_time']);
            $ret['status']['timeout'] = WPvivid_Time::format_local("M d, Y H:i", $ret['status']['timeout']);
        ...
        $message=$ret;
        if(empty($message)){
            $last_message=__('The last backup message not found.', 'wpvivid-backup-pro');
        }
        else{
            $localtime = strtotime($message['status']['start_time']);
            if($message['status']['str'] == 'completed'){
                $backup_status='Succeeded';
                $last_message=$backup_status.', (Local Time) '.WPvivid_Time::format_local("l, F-d-Y H:i", $localtime);
            }
    ...
    public function wpvivid_get_incremental_output_msg(){
        ...
        if(empty($message))
        {
            ...
            else if($message['status']['str'] == 'cancel')
            {
                ...
                $last_message=__('N/A.', 'wpvivid-backup-pro');
            }
        }
    ...
    public function wpvivid_mainwp_render_log_last_lines($filename, $lines = 3)
    {
        ...
        $last_line = $file->key();
        $offset = max(0, $last_line - $lines + 1);
        $data = array();
        $file->seek($offset);
        while(!$file->eof())
        {
