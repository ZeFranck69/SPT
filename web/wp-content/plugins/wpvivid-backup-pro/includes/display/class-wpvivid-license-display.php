...
class WPvivid_Pro_License
{
    public function __construct()
    {
        ...
        add_action('wpvivid_output_license_page', array($this, 'init_page'));
        //
    }
    ...
    public function get_dashboard_menu($submenus,$parent_slug)
    {
        ...
        if($display)
        {
            if(apply_filters('wpvivid_show_dashboard_addons',true))
            {
                ...
                $submenu['menu_title'] = __('License', 'wpvivid-backup-pro');
    ...
    public function welcome_bar()
    {
        ...
        <div class="wpvivid-welcome-bar wpvivid-clear-float">
            <div class="wpvivid-welcome-bar-left">
                <p><span class="dashicons dashicons-admin-network wpvivid-dashicons-large wpvivid-dashicons-green"></span><span class="wpvivid-page-title"><?php esc_html_e('License', 'wpvivid-backup-pro'); ?></span></p>
                <span class="about-description"><?php printf(esc_html__('This tab allows you to activate %s Pro license and get plugin updates and support.', 'wpvivid-backup-pro'), esc_html(apply_filters('wpvivid_white_label_display', 'WPvivid'))); ?></span>
            </div>
    ...
    public function status_bar()
    {
        ...
        <div class="wpvivid-two-col">
            <p>
                ...
                <span><?php esc_html_e('Current Version', 'wpvivid-backup-pro'); ?>: </span><span><?php echo $current_version; ?></span>
                <span><?php echo $version_compare; ?></span>
            </p>
            <p>
                <span class="dashicons dashicons-update-alt wpvivid-dashicons-blue"></span>
                <span><?php esc_html_e('Automatic Updates', 'wpvivid-backup-pro'); ?>: </span>
                <span id="auto_update_status"><?php echo esc_html($auto_update_status); ?></span>
                <span class="wpvivid-rectangle <?php echo esc_attr($auto_update_class); ?>" id="wpvivid_auto_update_switch" title="Click here to disable automatic updates of WPvivid Plugin" style="cursor:pointer;">
                                                        <?php echo esc_html($auto_update_text); ?>
                                                    </span>
            </p>
            <p>
                <span class="dashicons dashicons-yes-alt wpvivid-dashicons-blue"></span>
                <span><?php esc_html_e('Status', 'wpvivid-backup-pro'); ?>: </span>
    ...
    public function sign_out_bar()
    {
        if(isset($_REQUEST['sign_out']))
        {
            ...
            $url = 'admin.php?page=wpvivid-license';
            ?>
            <script>
                location.href='<?php echo $url;?>';
            </script>
        ...
        <span><a href="<?php echo esc_html($white_label_website_protocol); ?>://<?php echo esc_html($white_label_website); ?>" target="_blank"><?php esc_html_e('My Account', 'wpvivid-backup-pro'); ?></a></span>
        <span class="dashicons dashicons-migrate wpvivid-dashicons-grey"></span>
        <span><a href="#" id="wpvivid_dashboard_signout"><?php esc_html_e('Sign Out', 'wpvivid-backup-pro'); ?></a></span>
        <span class="dashicons dashicons-editor-help wpvivid-dashicons-editor-help wpvivid-tooltip">
            <div class="wpvivid-bottom">
                <!-- The content you need -->
                <p><?php esc_html_e('Sign out or switch to another account. Once signed out, you will need to re-enter the credentials to get WPvivid Pro authorization.', 'wpvivid-backup-pro'); ?></p>
                <i></i> <!-- do not delete this line -->
            </div>
        </span>
        <script>
            ...
            {
                var descript = <?php echo wp_json_encode(__('Are you sure you want to sign out?', 'wpvivid-backup-pro')); ?>;
    ...
    public function login_form()
    {
        ...
        <form action="">
            <div style="margin-top: 10px; margin-bottom: 15px;">
                <input type="password" class="regular-text" id="wpvivid_account_license" placeholder="<?php esc_attr_e('License', 'wpvivid-backup-pro'); ?>" autocomplete="new-password" required="">
            </div>
            <div style="margin-bottom: 10px; float: left; margin-left: 0; margin-right: 10px;">
                <input class="button-primary" id="wpvivid_active_btn" type="button" value="<?php esc_attr_e('Activate', 'wpvivid-backup-pro'); ?>">
            </div>
            <div style="clear:both;"></div>
        ...
        <script>
            ...
            function wpvivid_dashboard_login()
            {
                ...
                wpvivid_post_request_addon(ajax_data, function(data)
                {
                    ...
                    if (jsonarray.result === 'success')
                    {
                        ...
                        {
                            retry_times=0;
                            wpvivid_login_progress(<?php echo wp_json_encode(__('You have successfully logged in', 'wpvivid-backup-pro')); ?>);
                            wpvivid_active_site();
                        }
                        else
                        {
            ...
            function wpvivid_active_site()
            {
                ...
                wpvivid_lock_login(true);
                wpvivid_login_progress(<?php echo wp_json_encode(__('Activating your license on the current site', 'wpvivid-backup-pro')); ?>);
                jQuery('#wpvivid_pro_notice').hide();
                wpvivid_post_request_addon(ajax_data, function(data)
                {
                    var jsonarray = jQuery.parseJSON(data);
                    if (jsonarray.result === 'success')
                    {
                        wpvivid_login_progress(<?php echo wp_json_encode(__('Your license has been activated successfully', 'wpvivid-backup-pro')); ?>);
                        location.reload();
                    }
                    else
                    {
    ...
    public function logged()
    {
        ...
        if(empty($dashboard_info))
        {
            ...
            ?>
            <p><input id="wpvivid_check_update_plugin" type="button" class="button-primary ud_connectsubmit" value="<?php esc_attr_e('Check Update', 'wpvivid-backup-pro'); ?>"></p>
            ...
            <script>
                ...
                function wpvivid_check_update_plugin($slug)
                {
                    ...
                    jQuery('#wpvivid_user_info_log_progress_text').html(<?php echo wp_json_encode(__('Checking Update...', 'wpvivid-backup-pro')); ?>);
    ...
    public function plugin_list()
    {
        ...
        ?>
        <div>
            <p><?php printf(wp_kses_post(__('Click <b>Uninstall</b> to deactivate and remove the addon.', 'wpvivid-backup-pro'))); ?></p>
        </div>
        <table class="widefat" style="margin-top:1em;">
            <thead>
            <tr>
                <th><?php esc_html_e('Addons', 'wpvivid-backup-pro'); ?></th>
                <th><?php esc_html_e('Status', 'wpvivid-backup-pro'); ?></th>
                <th style="text-align:center;"><?php esc_html_e('Current Version/Latest Version', 'wpvivid-backup-pro'); ?></th>
                <th><?php esc_html_e('Update', 'wpvivid-backup-pro'); ?></th>
                <th><?php esc_html_e('Remove', 'wpvivid-backup-pro'); ?></th>
            </tr>
            </thead>
            <tbody>
            <?php
            foreach ($plugins as $slug=>$item)
            {
                ...
                <tr>
                    ...
                    if($item['delete'])
                    {
                        ?><td><a data-id="<?php echo $item['plugin_slug'];?>" class="wpvivid_dashboard_remove_plugin" href="#"><?php esc_html_e('Uninstall', 'wpvivid-backup-pro'); ?></a></td><?php
                    }
                    else
                    {
                        ?><td><a></a></td><?php
        ...
        <script>
            ...
            function wpvivid_dashboard_remove_plugin(obj)
            {
                ...
                obj.html(<?php echo wp_json_encode(__('Uninstalling...', 'wpvivid-backup-pro')); ?>);
                ...
                {
                    jQuery('.wpvivid_dashboard_remove_plugin').css({'pointer-events': 'auto', 'opacity': '1'});
                    obj.html(<?php echo wp_json_encode(__('Uninstall', 'wpvivid-backup-pro')); ?>);
                });
            }
        </script>
        <?php
    ...
    public function dashboard_login()
    {
        ...
        try
        {
            if(isset($_POST['license']))
            {
                ...
                {
                    $ret['result']='failed';
                    $ret['error']=__('A license is required.', 'wpvivid-backup-pro');
