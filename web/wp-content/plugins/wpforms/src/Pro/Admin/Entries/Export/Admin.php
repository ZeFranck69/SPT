...
class Admin {
	...
	public function display_additional_info_block() {
		...
		foreach ( $additional_info_fields as $slug => $label ) {
			...
			if (
				$slug === 'pginfo'
				&& ! ( class_exists( 'WPForms_Paypal_Standard' ) || class_exists( '\WPFormsStripe\Loader' ) || class_exists( '\WPFormsAuthorizeNet\Loader' ) || class_exists( '\WPFormsSquare\Plugin' ) || class_exists( '\WPFormsPaypalCommerce\Plugin' ) || class_exists( '\WPFormsMercadoPago\Loader' ) )
			) {
				continue;
			}
